...
(function (root) {
  ...
  var RELEASES = [
    { v: '0.200.0', d: '2026-09-25', area: 'fleet',
      en: { t: 'One screen for the agents, one for the sites', s: 'The estate was spread over six screens that described two things. Fleet listed every agent with a health verdict; Agents listed the same agents with a different one, worked out in the browser from how long ago each had reported. Interfaces was in the fleet menu but could only ever show one agent, behind a dropdown, so it could not answer which ports in the estate are dropping frames. Sites drew the locations on a map and Locations was the same records with the buttons that change them, each screen carrying a link to the other. There is one Fleet screen now: the same rows of agents throughout, with a switch for what you want to know about them — the measurements, the deployment (version, source, data quality), or the ports and NICs. Your filter, your search and your sort survive the switch, because the rows never change. Clicking one opens the agent beside the list: why it has the verdict it has, the evidence behind it, every port with its errors and discards, its NIC cards, and what the machine is — without losing your place. Sites is one screen with the map and the register as two tabs, and the NIC firmware inventory moved to Administration, where asset data belongs.' },
      da: { t: 'Én skærm til agenterne, én til lokationerne', s: 'Flåden lå spredt over seks skærme, der beskrev to ting. Fleet viste alle agenter med en sundhedsvurdering; Agenter viste de samme agenter med en anden, regnet ud i browseren ud fra hvor længe siden hver enkelt havde rapporteret. Interfaces lå i flådemenuen, men kunne kun vise én agent ad gangen bag en rulleliste, så den kunne ikke svare på, hvilke porte i flåden der taber frames. Lokationer tegnede sitene på et kort, og Lokationsregistret var de samme poster med knapperne, der ændrer dem — hver skærm med et link til den anden. Nu er der én Fleet-skærm: de samme rækker af agenter hele vejen, med et valg af hvad du vil vide om dem — målingerne, udrulningen (version, kilde, datakvalitet) eller porte og NIC’er. Dit filter, din søgning og din sortering overlever skiftet, fordi rækkerne aldrig ændrer sig. Et klik åbner agenten ved siden af listen: hvorfor den har den vurdering, den har, beviserne bag, hver port med fejl og discards, dens NIC-kort og hvad maskinen er — uden at du mister din plads. Lokationer er én skærm med kortet og registret som to faner, og NIC-firmwareinventaret er flyttet til Administration, hvor aktivdata hører hjemme.' } },
    { v: '0.185.0', d: '2026-09-21', area: 'platform',
      en: { t: 'Errors that tell you what to do about them', s: 'When the AI assistant could not answer, the dashboard said "Internal Server Error" \u2014 the same thing it says for a genuine bug \u2014 and threw away the sentence that told you a key was missing and where to set it. A provider that does not answer and a server that was never configured are now two different answers, each with its reason and each written to the system log. Re-pinning an agent that is too old to understand the command no longer looks like a refusal; you get the command to run on the host instead. And a stop on a traceroute path is no longer a bare country code: it shows the networks it crosses and its slowest hop, and clicking it lists every router \u2014 address, network, latency, loss and why it was flagged.' },
      da: { t: 'Fejl, der fort\u00e6ller hvad du skal g\u00f8re ved dem', s: 'N\u00e5r AI-assistenten ikke kunne svare, sagde dashboardet \u201cInternal Server Error\u201d \u2014 det samme som ved en \u00e6gte fejl \u2014 og smed den s\u00e6tning v\u00e6k, der fortalte at der manglede en n\u00f8gle, og hvor den skulle s\u00e6ttes. En udbyder, der ikke svarer, og en server, der aldrig blev sat op, er nu to forskellige svar, hvert med sin \u00e5rsag og begge skrevet i systemloggen. At fastl\u00e5se en agent, der er for gammel til at forst\u00e5 kommandoen, ligner ikke l\u00e6ngere en afvisning; du f\u00e5r i stedet kommandoen, der skal k\u00f8res p\u00e5 v\u00e6rten. Og et stop p\u00e5 en traceroute-rute er ikke l\u00e6ngere en bar landekode: det viser de netv\u00e6rk, det krydser, og sit langsomste hop, og et klik viser hver router \u2014 adresse, netv\u00e6rk, svartid, tab og hvorfor den blev markeret.' } },
    { v: '0.184.0', d: '2026-09-21', area: 'fleet',
      en: { t: 'When an agent refuses an update, the fix is one click away', s: 'An agent that will not take an update almost always means one thing: it trusts a different signing key than this server signs with, because the key was regenerated after that agent was installed. The dashboard could already fix that in one click and did not offer it, because it did not recognise the wording the agent actually uses. It does now, it shows you both keys side by side so you can see they differ, and it keeps that offer away from the refusals a re-pin would not fix \u2014 a server with no signing key, a host whose clock is wrong. The Windows updater also stopped needing two goes: it waited two seconds for the old agent to exit and then carried on regardless, so the first run half-updated and the second one worked. It now waits until the old process is really gone, and checks the new one actually came back up before it says it worked.' },
      da: { t: 'N\u00e5r en agent afviser en opdatering, er l\u00f8sningen \u00e9t klik v\u00e6k', s: 'En agent, der ikke vil tage en opdatering, betyder n\u00e6sten altid \u00e9t: den har fastl\u00e5st en anden signeringsn\u00f8gle end den, serveren signerer med, fordi n\u00f8glen blev oprettet igen efter agenten blev installeret. Dashboardet kunne allerede l\u00f8se det med \u00e9t klik og tilb\u00f8d det ikke, fordi det ikke genkendte den formulering, agenten faktisk bruger. Det g\u00f8r det nu, det viser begge n\u00f8gler ved siden af hinanden, s\u00e5 du kan se at de er forskellige, og det holder tilbuddet v\u00e6k fra de afvisninger, en ny fastl\u00e5sning ikke l\u00f8ser \u2014 en server uden signeringsn\u00f8gle, en v\u00e6rt med forkert ur. Windows-opdateringen holdt ogs\u00e5 op med at kr\u00e6ve to fors\u00f8g: den ventede to sekunder p\u00e5 at den gamle agent stoppede og fortsatte s\u00e5 uanset hvad, s\u00e5 f\u00f8rste fors\u00f8g opdaterede halvt og andet fors\u00f8g virkede. Nu venter den til den gamle proces virkelig er v\u00e6k, og tjekker at den nye faktisk kom op, f\u00f8r den siger at det lykkedes.' } },
    { v: '0.184.0', d: '2026-09-21', area: 'insights',
      en: { t: 'Analysis no longer slows down as findings pile up', s: 'The screen counted every finding the server had ever recorded each time you opened it \u2014 a hundred and eighty-five thousand of them on a fleet with ninety-eight open \u2014 so it got slower the longer the server ran, and accepting findings, the one thing offered to clean it up, could not help: an accepted finding is still a row. It now reads what is still open, which is the question the page asks in its title, and there is an index for exactly that. The full counts including everything accepted are still one dropdown away.' },
      da: { t: 'Analyse bliver ikke l\u00e6ngere langsommere, n\u00e5r fund hober sig op', s: 'Sk\u00e6rmen t\u00e6llede alle fund, serveren nogensinde har registreret, hver gang du \u00e5bnede den \u2014 hundrede og femogfirs tusinde p\u00e5 en fl\u00e5de med otteoghalvfems \u00e5bne \u2014 s\u00e5 den blev langsommere, jo l\u00e6ngere serveren k\u00f8rte, og at acceptere fund, det ene der blev tilbudt for at rydde op, kunne ikke hj\u00e6lpe: et accepteret fund er stadig en r\u00e6kke. Den l\u00e6ser nu det, der stadig er \u00e5bent \u2014 sp\u00f8rgsm\u00e5let siden stiller i sin egen titel \u2014 og der er et indeks til pr\u00e6cis det. De fulde tal inklusive alt accepteret er stadig \u00e9n rulleliste v\u00e6k.' } },
    { v: '0.183.0', d: '2026-09-21', area: 'fleet',
      en: { t: 'Accept clears what you accepted, and a community has a name', s: 'Pressing Accept on a host wrote thousands of rows and the screen did not move: the overview counted every finding a host ever had, accepted or not, so the one number that should have gone to zero never did. It now counts what is still open, and the row leaves the list when nothing is left. The pickers that take several answers — the sites a community is valid at, the agents allowed to use it, the vantage points a scan runs from — show what is selected as removable tags instead of a scrolling box where the highlight is the only clue. An agent that polls a switch itself now picks a named community from Settings instead of having the string typed into its config, so the secret stays in one place and stops being readable from the agents API. And when an agent refuses an update, the dashboard says why it refused rather than only that it did.' },
      da: { t: 'Accept rydder det, du accepterede \u2014 og en community har et navn', s: 'Et tryk p\u00e5 Accept skrev tusindvis af r\u00e6kker, og sk\u00e6rmen flyttede sig ikke: oversigten t\u00e6llede alle fund en v\u00e6rt nogensinde har haft, accepteret eller ej, s\u00e5 det ene tal der skulle v\u00e6re g\u00e5et i nul aldrig gjorde det. Den t\u00e6ller nu det, der stadig er \u00e5bent, og r\u00e6kken forsvinder fra listen, n\u00e5r der ikke er mere tilbage. De v\u00e6lgere, der tager flere svar \u2014 de lokationer en community g\u00e6lder p\u00e5, de agenter der m\u00e5 bruge den, de udsigtspunkter en scanning k\u00f8rer fra \u2014 viser det valgte som m\u00e6rkater, man kan fjerne, i stedet for en rulleboks hvor markeringen er det eneste spor. En agent, der selv poller en switch, v\u00e6lger nu en navngivet community fra Indstillinger i stedet for at f\u00e5 strengen skrevet ind i sin konfiguration, s\u00e5 hemmeligheden bliver \u00e9t sted og ikke l\u00e6ngere kan l\u00e6ses fra agent-API\u2019et. Og n\u00e5r en agent afviser en opdatering, siger dashboardet hvorfor den afviste \u2014 ikke kun at den gjorde det.' } },
    ...
    { v: '0.172.0', d: '2026-09-20', area: 'insights',
    ...
    { v: '0.172.0', d: '2026-09-20', area: 'assurance',
    ...
    { v: '0.172.0', d: '2026-09-20', area: 'fleet',
