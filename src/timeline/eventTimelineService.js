...
// Orchestrates the event-CLUSTER timeline: resolves the cluster, hydrates its
// member findings, derives the affected agents + event onset, fans out to
// every source per agent, and merges via the pure buildEventTimeline.
//
...
//   b. cluster lifecycle transitions (hash-chained audit_log, category event)
...
function createEventClusterTimelineService({
  clustersRepo,
  findingStore = null,
  ...
  // a STRING, but audit_events.actor_id, events.agent_id and
  ...
  async function fetchEvents(agentId, { from, to, limit }) {
    if (!probeOutagesRepo || typeof probeOutagesRepo.listForAgent !== 'function') return [];
    return probeOutagesRepo.listForAgent(toNum(agentId), { from, to, limit });
  }
  ...
  async function fetchStatusChanges(clusterId) {
    ...
    const rows = await auditLogRepo.listByTarget({ category: ['event', 'incident'], target: String(clusterId) });
  ...
  async function getTimeline(clusterId, { lookbackMinutes = 30, now = () => new Date() } = {}) {
    ...
    // a. member findings (also the source of affected agents + event onset).
    let members = [];
    try {
    ...
    // Event onset = earliest member finding; fall back to the cluster's own
    ...
    const jobs = [];
    const perAgent = new Map(agentIds.map((id) => [id, { agentId: id, agentEvents: [], playbookRuns: [], events: [], configChanges: [] }]));
    const failedNames = new Set();

    for (const id of agentIds) {
      jobs.push(['agentEvents', id, fetchAgentEvents(id, window)]);
      jobs.push(['playbookRuns', id, fetchPlaybookRuns(id, window)]);
      jobs.push(['events', id, fetchEvents(id, window)]);
    ...
    const { events, whatChanged } = buildEventTimeline({
...
module.exports = { createEventClusterTimelineService, AGENT_LIFECYCLE_ACTIONS };
