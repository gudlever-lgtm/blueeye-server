...
| 056 · 057 | `incident_clusters` (+ `advisory` col) | **cross-agent incident clusters** — findings from ≥2 agents grouped with a confidence tier + suspected common cause; `advisory` holds the opt-in cluster-level AI root-cause suggestion. See `docs/cross-agent-correlation.md` |
| 058 | `alert_dispatch_log` | durable record of dispatched alerts (finding + cluster) — lets a cluster alert fire once and reference (not resend) member-finding alerts |
