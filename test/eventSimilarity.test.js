...
const { scoreSimilarEvents, DEFAULT_WEIGHTS } = require('../src/eventCases/similarity');
...
test('scoring ranks by number/weight of matched criteria (known fixtures)', () => {
  ...
  const out = scoreSimilarEvents(TARGET, candidates);
...
test('candidates with no shared criteria are dropped', () => {
  const out = scoreSimilarEvents(TARGET, [
...
test('device match and device-type never double-count', () => {
  const [r] = scoreSimilarEvents(TARGET, [{ id: 6, hostId: '9', platform: 'linux', primaryMetric: 'x' }]);
...
test('the target itself is excluded and limit is honoured', () => {
  ...
  const out = scoreSimilarEvents(TARGET, cands, { limit: 2 });
  assert.equal(out.length, 2);
  assert.ok(!out.some((r) => r.id === 100));
});

test('weights are overridable', () => {
  const [r] = scoreSimilarEvents(TARGET, [{ id: 1, hostId: '9', primaryMetric: 'cpu' }], { weights: { device: 10 } });
