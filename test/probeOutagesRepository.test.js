...
const { createProbeOutagesRepository } = require('../src/repositories/probeOutagesRepository');
const { createProbeThresholdsRepository } = require('../src/repositories/probeThresholdsRepository');
...
// ---- probeOutagesRepository ---------------------------------------------------

test('outages.list maps joined rows to the API shape and derives status', async () => {
  const pool = fakePool(() => [[
    {
  ...
  const repo = createProbeOutagesRepository({ pool });
...
test('outages.list without a window omits the time clauses (no `<= NULL` matching nothing)', async () => {
...
test('outages.findActive returns the open row for the tuple or null', async () => {
...
test('outages.resolve issues a guarded UPDATE computing duration in SQL', async () => {
  const pool = fakePool((sql) => {
    assert.match(sql, /UPDATE probe_outages/);
...
test('outages.updateSeverity issues a guarded UPDATE on active rows', async () => {
  const pool = fakePool((sql) => {
    assert.match(sql, /UPDATE probe_outages SET severity/);
    assert.match(sql, /resolved_at IS NULL/);
    return [{ affectedRows: 1 }];
  });
  const repo = createProbeOutagesRepository({ pool });
...
test('thresholds.upsert updates an existing global (select-then-update path)', async () => {
  ...
  const pool = fakePool((sql) => {
    step += 1;
    if (step === 1) { assert.match(sql, /SELECT id FROM probe_thresholds/); assert.match(sql, /location_id IS NULL/); return [[{ id: 2 }]]; }
    if (step === 2) { assert.match(sql, /UPDATE probe_thresholds SET/); return [{ affectedRows: 1 }]; }
...
test('thresholds.upsert inserts a new location override when none exists', async () => {
  ...
  const pool = fakePool((sql) => {
    ...
    if (step === 2) { assert.match(sql, /INSERT INTO probe_thresholds/); return [{ insertId: 9 }]; }
