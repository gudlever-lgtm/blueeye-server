...
// "Where did this happen?" — the one place that turns an incident's device
// identity into text a human can act on. Pure, no I/O.
//
// An incident's `host_id` is the agent id, which on its own ("on 1") tells an
// operator nothing about which machine or which site to walk to. Everywhere an
// incident is named — the auto-generated title, the explanation's `where`, the
...
// Fallback used when the agent behind an incident can no longer be named.
