...
//
//   GET  /api/event-clusters          viewer+   list (filter status/time, paged)
//   GET  /api/event-clusters/:id      viewer+   full cluster: members, evidence,
//                                                   confidence breakdown, root cause
//   POST /api/event-clusters/:id/ack      operator+  acknowledge (audited)
//   POST /api/event-clusters/:id/resolve  operator+  resolve + required note (audited)
//
// NOTE ON THE MOUNT PATH: the task asked for `/api/events`, but that path is
// already the first-class `event_cases` router (a DISTINCT feature). Clusters
// therefore live at `/api/event-clusters`, matching how CODEMAP already names
...
// hash-chained audit_log via the injected auditLogger (as src/routes/events.js).
...
function createEventClustersRouter({
  ...
  // GET /api/event-clusters — filterable, paginated list. viewer+.
  ...
  // GET /api/event-clusters/:id — full cluster with members, evidence,
  ...
  // "what changed" slice of the pre-event lookback window. viewer+.
  router.get('/:id/timeline', requireAuth, reader, asyncHandler(async (req, res) => {
    ...
    if (!result) return res.status(404).json({ error: 'Event cluster not found' });
    return res.json(result);
  }));

  // GET /api/event-clusters/:id/recommended-actions — runbooks matching the
  ...
  // POST /api/event-clusters/:id/run-playbook — operator+. Explicit, audited
  ...
  router.post('/:id/run-playbook', requireAuth, writer, asyncHandler(async (req, res) => {
    ...
    if (auditLogger) {
      await auditLogger.record(req, {
        category: 'event', action: 'playbook_run', target: String(id),
  ...
  // GET /api/event-clusters/:id/notifications — the cluster's rollup state:
  ...
  // GET /api/event-clusters/:id/evidence — the read-only evidence snapshots
  ...
  // GET /api/event-clusters/:id/evidence/:sid — the raw-text payload of one
  ...
  // POST /api/event-clusters/:id/evidence — manual re-snapshot (operator+),
  ...
  router.post('/:id/evidence', requireAuth, writer, asyncHandler(async (req, res) => {
    ...
    if (auditLogger) {
      await auditLogger.record(req, {
        category: 'event', action: 'evidence_resnapshot', target: String(id),
  ...
  // POST /api/event-clusters/:id/ack — acknowledge (operator+). Audited.
  router.post('/:id/ack', requireAuth, writer, asyncHandler(async (req, res) => {
    const id = parseId(req.params.id);
    if (id === null) return res.status(400).json({ error: 'id must be a positive integer' });

    const existing = await clustersRepo.findById(id);
    if (!existing) return res.status(404).json({ error: 'Event cluster not found' });
    ...
    if (auditLogger) {
      await auditLogger.record(req, {
        category: 'event', action: 'cluster_acknowledge', target: String(id),
...
module.exports = { createEventClustersRouter };
