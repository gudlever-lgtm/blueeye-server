...
| `/api/targets` | targets.js | viewer+ | **per-target (per-agent) incident timeline**: `GET /:id/timeline?from&to&limit` — merges anomaly findings, probe-outage incidents, agent connect/disconnect (`audit_events`) and remediation playbook runs into one chronological (newest-first) list, each item `{ timestamp, source, type, severity, summary, ref_id }`. Default last 24h, `limit` capped 500. Read-only, no new tables. Partial-failure tolerant (one source down → `partial:true` + `failedSources`, not a 500). Pure merge in `src/timeline/targetTimeline.js` (reusable), fan-out in `src/timeline/targetTimelineService.js`. See `docs/target-timeline.md` |
...
| 057 · 058 | `incident_clusters` (+ `advisory` col) | **cross-agent incident clusters** — findings from ≥2 agents grouped with a confidence tier + suspected common cause; `advisory` holds the opt-in cluster-level AI root-cause suggestion. See `docs/cross-agent-correlation.md` |
| 060 | `incident_clusters` lifecycle cols | **cluster operator lifecycle** — `status` enum gains `acknowledged`; adds `acknowledged_at`/`acknowledged_by`, `resolved_by`, `resolution_note` (`*_by`→users, ON DELETE SET NULL). Backs the ack/resolve REST API |
...
| 064 | `incident_clusters` rollup refs | **cluster-level notification rollup** — `alert_last_at`/`alert_last_severity`/`alert_member_count`, `itsm_ticket_ref`/`itsm_integration_id` (one ITSM ticket per cluster), `nis2_draft_id` (one NIS2 draft per cluster). See CHANGELOG 0.83.0 |
...
| Cross-agent incident clusters | `src/analysis/crossAgentCorrelator.js` (pure detector — groups findings from ≥2 agents in a time window; confidence: time=low, +shared site `location_id`=medium, +same finding-type=high — subnet/VLAN/LLDP don't exist, shared-site is the only topology signal) + `src/analysis/crossAgentClusterService.js` (detect+dedup+resolve + opt-in **cluster-level AI advisory** for medium/high clusters via `assistant.suggestClusterCause`, stored in `incident_clusters.advisory`, always published with its evidence list; best-effort) + `src/analysis/crossAgentClusterJob.js` (leader-only ~60s sweep in `server.js` `backgroundJobs`) + opt-in **cluster-level alerting** (medium/high) that fires ONCE per cluster via `dispatcher.dispatchCluster`, referencing (not resending) member alerts through the durable `alert_dispatch_log` (`repositories/alertDispatchLogRepository.js`, migration 059) + `repositories/incidentClustersRepository.js`; tables `incident_clusters` (migration 057 + `advisory` in 058). Cluster events reuse the `/ws/dashboard` broadcast (`{type:'incident_cluster'}`). Fakes `makeIncidentClustersRepo`/`makeAlertDispatchLogRepo`. **Distinct** from the per-device `incident_cases` and the per-target `correlator.js`. **Operator REST API + lifecycle:** router `src/routes/incidentClusters.js` (`/api/incident-clusters` — list/get/ack/resolve, RBAC + hash-chained audit), pure detail read-model `src/analysis/clusterView.js` (member hydration + `confidenceBreakdown` + network/application root-cause layer via the L2 `isAppMetric`/`isNetMetric`), `status` gains `acknowledged` + ack/resolve columns (migration 060); the auto-resolve sweep NEVER closes a cluster with an unacknowledged CRIT member and uses a 30-min default quiet period. **Automated read-only evidence on cluster open (Fase 6):** `src/evidence/` — `commandAllowlist.js` (`evidence-v1`, the single source of truth for the read-only items `iface.counters`/`arp.table`/`snmp.reads`/`agent.state`), `snapshotService.js` (captures over the existing `agentCommander.sendCommandAndWait` path — 30s/target timeout, concurrency 4, one 60s offline retry, Ed25519-signed with the release key, fire-and-forget from the sweep, NEVER blocks), `evidenceRetention.js` (6h age-out, skips unacked-CRIT clusters, default 90d) + `repositories/evidenceSnapshotsRepository.js` (gzip blob per (cluster, target), table `cluster_evidence_snapshots` mig 065); the agent enforces its OWN read-only allowlist (`blueeye-agent` `src/evidenceCollector.js`, defense in depth). API `GET/POST /api/incident-clusters/:id/evidence(/:sid)`; timeline `evidence` source in `src/timeline/incidentTimeline.js`. Fake `makeEvidenceSnapshotsRepo`. See `docs/cross-agent-correlation.md` |
