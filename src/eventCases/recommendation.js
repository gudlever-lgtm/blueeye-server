...
// Pure assembly for the unified event recommendation
// (GET /api/events/:id/recommendation). The route fetches the rows; these
// helpers shape the three sections, ALWAYS in this order:
//   (a) matching_playbook  — a playbook matched on the event's anomaly-type
//   (b) historical_matches — earlier RESOLVED events (Fase-4 similarity)
...
// null). `runs` are the runs already recorded against THIS event (newest
// first). When the matched playbook has already been run on this event we show
...
// (b) The historical_matches section. `ranked` are the scored similar events
// (already RESOLVED-only, from the reused similarity logic). For each we add:
//   - playbook: which playbook (if any) was used on that past event
//   - resolutionTimeSeconds: how long it took to resolve
//   - timesSeen: how many resolved events share this anomaly-type (pattern
...
// Picks the run to surface for a past event: a succeeded run wins, else the
...
function buildHistoricalMatches(ranked = [], { runsByEvent = {}, resolvedCandidates = [] } = {}) {
  ...
  return (Array.isArray(ranked) ? ranked : []).map((r) => {
    ...
    return {
      ...
      playbook: playbookForHistorical(runsByEvent[r.id]),
    };
  });
}
