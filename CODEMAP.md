...
| `/api/transactions` | transactions.js | viewer+ read / admin write | **transaction tests** (http/tcp/dns) — CRUD + `PUT /:id/agents` (assign) + `GET /:id/results?from&to&agent_id` + `GET /:id/heatmap?from&to&bucket` (SQL-aggregated avg_latency/fail_count/sample_count per bucket per agent). Agents get their config + report results over `/ws/agent` (`transaction_config`/`transaction_result`); threshold alerts reuse the alerting dispatcher. Repo `src/repositories/transactionsRepository.js`, validation `src/validation/transactionValidation.js`, alert eval `src/analysis/transactionAlerts.js` |
...
| 046 | `transaction_tests` · `transaction_test_agents` · `transaction_results` | **transaction tests** (http/tcp/dns journeys) — normalized: one test row (type + JSON config + thresholds), an agent-assignment join, and one result row per (test, agent, run). Config is pushed to agents over WS and results ingested over WS. Supersedes the dead-in-prod migration 045 (dropped here) |
