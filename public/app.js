...
// Phase → human-readable diagnosis text.
const TX_PHASE_LABELS = {
  dns:     'DNS-opslag mislykkedes — hostnavnet kan ikke løses',
  connect: 'TCP-forbindelsen fejlede — netværk / firewall / host nede',
  tls:     'TLS-håndtryk mislykkedes — certifikat ugyldig eller udløbet',
  send:    'HTTP-anmodning kunne ikke sendes — forbindelsen afbrudt',
  recv:    'Svar modtaget ikke — forbindelsen timeout eller nulstillet',
  status:  'Uventet HTTP-statuskode — server svarede med fejl',
  keyword: 'Forventet nøgleord ikke fundet i svaret',
  extract: 'Udtræk mislykkedes — regex matchede ikke',
  timeout: 'Anmodningen tog for lang tid (timeout)',
};

function txDiagnose(error_detail, stepIndex) {
  if (!error_detail) return null;
  const phase = error_detail.phase || '';
  const code  = error_detail.code  ? ` (${error_detail.code})` : '';
  const step  = stepIndex != null ? `Trin ${stepIndex + 1}` : 'Testen';
  const label = TX_PHASE_LABELS[phase] || `Fejl i fasen "${phase}"`;
  return `${step} fejlede i ${phase}-fasen${code} — ${label.replace(/^[^—]+—\s*/, '')}`;
}

function txStatusBadge(status) {
  const cls = status === 'ok' ? 'active' : (status === 'fail' ? 'warn' : 'danger');
  return el('span', { class: `badge ${cls}` }, status || '?');
}

function txDeviationArrow(deviation) {
  if (deviation == null || Math.abs(deviation) < 0.1) return null;
  return el('span', { class: 'muted small', title: `${deviation > 0 ? '+' : ''}${Math.round(deviation * 100)}% vs. median` },
    deviation > 0 ? ' ↑' : ' ↓');
}

// SVG heatmap grid (no chart library).
function txHeatmapSvg({ rows, agentIds, buckets, maxLatency }) {
  if (!rows.length) return el('div', { class: 'empty' }, 'Ingen data i valgt periode.');
  const CW = 14, CH = 20, PL = 60, PT = 22, ROWS = agentIds.length, COLS = buckets.length;
  const W = PL + COLS * CW + 4, H = PT + ROWS * CH + 4;
  ...
  const cellMap = {};
  for (const r of rows) cellMap[`${r.agent_id}_${r.bucket}`] = r;

  const latColor = (avg) => {
    if (avg == null) return '#333';
    const t = Math.min(1, avg / (maxLatency || 2000));
    const r = Math.round(60 + t * 195), g = Math.round(200 - t * 170), b = 60;
    return `rgb(${r},${g},${b})`;
  };

  const tooltip = document.createElementNS(ns, 'title');

  agentIds.forEach((agId, row) => {
    const label = document.createElementNS(ns, 'text');
    label.setAttribute('x', PL - 4); label.setAttribute('y', PT + row * CH + CH * 0.7);
    label.setAttribute('text-anchor', 'end'); label.setAttribute('font-size', '10');
    label.setAttribute('fill', 'var(--text-muted, #888)');
    label.textContent = `Agent ${agId}`;
    svg.appendChild(label);

    buckets.forEach((bkt, col) => {
      const cell = cellMap[`${agId}_${bkt}`];
      const rect = document.createElementNS(ns, 'rect');
      rect.setAttribute('x', PL + col * CW); rect.setAttribute('y', PT + row * CH);
      rect.setAttribute('width', CW - 1); rect.setAttribute('height', CH - 1);
      rect.setAttribute('rx', 2);
      const hasFail = cell && cell.fails > 0;
      rect.setAttribute('fill', cell ? (hasFail ? '#1a0a0a' : latColor(cell.avg_latency)) : '#1e1e1e');
      if (cell) {
        const tip = document.createElementNS(ns, 'title');
        tip.textContent = `Agent ${agId} · ${cell.avg_latency != null ? Math.round(cell.avg_latency) + ' ms' : '—'} · ${cell.fails} fejl · ${cell.samples} kørsel(er)`;
  ...
  // X-axis bucket labels (every Nth bucket to avoid clutter)
  const step = Math.max(1, Math.floor(buckets.length / 8));
  buckets.forEach((bkt, col) => {
    if (col % step !== 0) return;
    const t = document.createElementNS(ns, 'text');
    t.setAttribute('x', PL + col * CW + CW / 2); t.setAttribute('y', PT - 4);
    t.setAttribute('text-anchor', 'middle'); t.setAttribute('font-size', '9');
    t.setAttribute('fill', 'var(--text-muted, #888)');
    const d = new Date(bkt * 1000);
...
// Trend SVG: one line per step, x = days, y = median latency.
function txTrendSvg({ stepSeries, days, testSteps }) {
  if (!stepSeries.length) return el('div', { class: 'empty' }, 'Ingen trend-data i valgt periode.');
  const W = 540, H = 160, PL = 50, PR = 10, PT = 10, PB = 28;
  const iW = W - PL - PR, iH = H - PT - PB;
  const ns = 'http://www.w3.org/2000/svg';
  const svg = document.createElementNS(ns, 'svg');
  svg.setAttribute('viewBox', `0 0 ${W} ${H}`);
  svg.style.cssText = 'width:100%;max-width:600px;display:block';

  const allVals = stepSeries.flatMap((s) => s.points.map((p) => p.median)).filter((v) => v != null);
  const maxV = allVals.length ? Math.max(...allVals) * 1.1 : 1000;
  const allDays = [...new Set(stepSeries.flatMap((s) => s.points.map((p) => p.day)))].sort();
  const xOf = (day) => PL + (allDays.indexOf(day) / Math.max(allDays.length - 1, 1)) * iW;
  const yOf = (v) => PT + iH - (v / maxV) * iH;

  const COLORS = ['#4a9eff', '#ff7043', '#66bb6a', '#ffa726', '#ab47bc', '#26c6da'];

  stepSeries.forEach((series, si) => {
    const color = COLORS[si % COLORS.length];
    // Baseline reference line (stipled)
    if (series.baseline != null) {
      const bl = document.createElementNS(ns, 'line');
      bl.setAttribute('x1', PL); bl.setAttribute('x2', PL + iW);
      bl.setAttribute('y1', yOf(series.baseline)); bl.setAttribute('y2', yOf(series.baseline));
      bl.setAttribute('stroke', color); bl.setAttribute('stroke-width', '1');
      bl.setAttribute('stroke-dasharray', '4 3'); bl.setAttribute('opacity', '0.45');
      svg.appendChild(bl);
    }
    // Trend line
    const pts = series.points.filter((p) => p.median != null);
    if (pts.length > 1) {
      const d = pts.map((p, i) => `${i === 0 ? 'M' : 'L'}${xOf(p.day).toFixed(1)},${yOf(p.median).toFixed(1)}`).join(' ');
    ...
    for (const p of pts) {
      ...
      c.setAttribute('cx', xOf(p.day).toFixed(1)); c.setAttribute('cy', yOf(p.median).toFixed(1));
      c.setAttribute('r', '3'); c.setAttribute('fill', color);
      const tip = document.createElementNS(ns, 'title');
      tip.textContent = `${series.name} · ${p.day}: ${Math.round(p.median)} ms`;
  ...
  yLabel.setAttribute('x', 4); yLabel.setAttribute('y', PT + iH / 2);
  yLabel.setAttribute('transform', `rotate(-90,4,${PT + iH / 2})`);
  yLabel.setAttribute('text-anchor', 'middle'); yLabel.setAttribute('font-size', '9');
  yLabel.setAttribute('fill', 'var(--text-muted,#888)');
  yLabel.textContent = 'ms';
  svg.appendChild(yLabel);

  // X-axis day labels
  allDays.forEach((day, i) => {
    if (i % Math.max(1, Math.floor(allDays.length / 6)) !== 0) return;
    const t = document.createElementNS(ns, 'text');
    t.setAttribute('x', xOf(day).toFixed(1)); t.setAttribute('y', H - 4);
    t.setAttribute('text-anchor', 'middle'); t.setAttribute('font-size', '9');
    t.setAttribute('fill', 'var(--text-muted,#888)');
    t.textContent = day.slice(5); // MM-DD
    svg.appendChild(t);
  });
...
let txTab = 'list'; // 'list' | 'matrix' | 'heatmap' | 'trend'
let txDetailId = null; // test ID selected for heatmap / trend detail

views.transactionTests = async () => {
  const root = el('div');
  const tab = (key, label) => el('button', {
    class: `small ghost${txTab === key ? ' active' : ''}`,
    onclick: () => { txTab = key; render(); },
  }, label);
  root.append(el('div', { class: 'section-head' },
    el('h2', {}, 'Transaktionstests'),
    el('div', { class: 'subtabs' },
      tab('list', 'Liste'),
      tab('matrix', 'Matrix'),
      tab('heatmap', 'Tidsheatmap'),
      tab('trend', 'Trend pr. trin'),
    )));

  try {
    if (txTab === 'list')    root.append(await txListView());
    if (txTab === 'matrix')  root.append(await txMatrixView());
    if (txTab === 'heatmap') root.append(await txHeatmapView());
    if (txTab === 'trend')   root.append(await txTrendView());
  } catch (err) {
    root.append(el('div', { class: 'error' }, `Fejl: ${errText(err)}`));
  }
  return root;
};
...
async function txListView() {
  const [tests, agents] = await Promise.all([
    api('/api/transaction-tests'),
    api('/agents').catch(() => []),
  ]);
  const root = el('div');
  root.append(el('div', { class: 'history-controls' },
    el('span', { class: 'muted' }, `${tests.length} test${tests.length === 1 ? '' : 's'} konfigureret`),
    el('span', { class: 'spacer' }),
    isAdmin() ? el('button', { class: 'small', onclick: () => txEditModal(null, agents) }, '+ Ny test') : null,
  ));
  if (!tests.length) {
    return el('div', { class: 'empty' }, 'Ingen transaktionstests endnu. En transaktionstest er en sekvens af HTTP-trin der køres fra tilknyttede agenter.');
  }
  const tbody = el('tbody');
  root.append(el('table', { class: 'tests-table' },
    el('thead', {}, el('tr', {}, ...['Navn', 'Type', 'Trin', 'Agenter', 'Status', 'Hemmeligheder', ''].map((h) => el('th', {}, h)))),
    tbody));
  tbody.append(...tests.map((t) => txListRow(t, agents)));
  return root;
}

function txListRow(t, agents) {
  const agentLabel = (Array.isArray(t.agents) && t.agents[0] === 'all')
    ? `Alle (${agents.length})` : `${(t.agents || []).length} agent(er)`;
  const secretLabel = (t.secret_names || []).length
    ? (t.secret_names || []).map((n) => el('span', { class: 'badge neutral', style: 'margin-right:2px' }, n)).reduce((a, b) => [a, b])
    : el('span', { class: 'muted' }, '–');
  return el('tr', {},
    el('td', {}, el('strong', {}, t.name)),
    el('td', {}, t.type || 'http'),
    el('td', {}, String((t.steps || []).length)),
    el('td', { class: 'muted' }, agentLabel),
    el('td', {}, txStatusBadge(t.enabled ? 'ok' : 'disabled').cloneNode(true),
      el('span', { class: `badge ${t.enabled ? 'active' : 'neutral'}` }, t.enabled ? 'aktiv' : 'deaktiveret')),
    el('td', {}, ...(Array.isArray(secretLabel) ? secretLabel : [secretLabel])),
    el('td', {}, el('div', { class: 'row-actions' },
      el('button', { class: 'small ghost', onclick: () => txShowDetail(t) }, 'Detalje'),
      isAdmin() ? el('button', { class: 'small ghost', onclick: () => txEditModal(t, agents) }, 'Rediger') : null,
      isAdmin() ? el('button', { class: 'small danger', onclick: () => txDelete(t) }, 'Slet') : null,
    )),
  );
}

async function txDelete(t) {
  if (!confirm(`Slet transaktionstest "${t.name}"?`)) return;
  try { await api(`/api/transaction-tests/${t.id}`, { method: 'DELETE' }); toast('Slettet'); render(); }
  catch (err) { toast(errText(err), true); }
}

async function txShowDetail(t) {
  txDetailId = t.id;
  txTab = 'trend';
  render();
}
...
async function txMatrixView() {
  const [matrix, agents] = await Promise.all([
    api('/api/transaction-tests/matrix'),
    api('/agents').catch(() => []),
  ]);
  const { tests, agent_ids, cells } = matrix;
  if (!tests.length) return el('div', { class: 'empty' }, 'Ingen transaktionstests endnu.');

  const agentMap = {};
  for (const a of agents) agentMap[a.id] = a.display_name || a.hostname || `Agent ${a.id}`;

  const root = el('div');
  root.append(el('p', { class: 'muted small' }, 'Seneste status pr. agent og test. Klik en celle for at se detaljer.'));

  const headerCells = [el('th', {}, 'Agent'), ...tests.map((t) => el('th', {}, t.name))];
  const tbody = el('tbody');
  for (const agId of agent_ids) {
    const tds = tests.map((t) => {
      const cell = cells[`${t.id}_${agId}`];
      if (!cell) return el('td', { class: 'muted', style: 'text-align:center' }, '–');
      const arrow = txDeviationArrow(cell.deviation);
      const badge = txStatusBadge(cell.status);
      const td = el('td', { style: 'text-align:center;cursor:pointer', onclick: () => { txDetailId = t.id; txTab = 'trend'; render(); } },
        badge, arrow,
        cell.duration_ms != null ? el('div', { class: 'muted small' }, `${cell.duration_ms} ms`) : null);
      return td;
    });
    tbody.append(el('tr', {}, el('td', {}, agentMap[agId] || `Agent ${agId}`), ...tds));
  }

  root.append(el('div', { style: 'overflow-x:auto' },
    el('table', { class: 'tests-table' }, el('thead', {}, el('tr', {}, ...headerCells)), tbody)));
  return root;
}

// ---- Heatmap tab -------------------------------------------------------------

async function txHeatmapView() {
  const tests = await api('/api/transaction-tests');
  if (!tests.length) return el('div', { class: 'empty' }, 'Ingen transaktionstests endnu.');

  const selId = txDetailId || tests[0].id;
  const testSel = el('select', {}, ...tests.map((t) => el('option', { value: String(t.id), ...(t.id === selId ? { selected: 'selected' } : {}) }, t.name)));
  testSel.addEventListener('change', () => { txDetailId = Number(testSel.value); render(); });
  const bucketSel = el('select', {}, ...['5m', '15m', '1h'].map((v) => el('option', { value: v }, v)));

  const root = el('div');
  const gridHost = el('div', { style: 'margin-top:12px;overflow-x:auto' });
  root.append(el('div', { class: 'history-controls' },
    el('label', { class: 'inline muted' }, 'Test ', testSel),
    el('label', { class: 'inline muted' }, 'Bucket ', bucketSel),
  ), gridHost);

  async function loadHeatmap() {
    try {
      const { rows } = await api(`/api/transaction-tests/heatmap?test_id=${selId}&bucket=${bucketSel.value}&hours=24`);
      const agentIds = [...new Set(rows.map((r) => r.agent_id))];
      const buckets  = [...new Set(rows.map((r) => r.bucket))].sort((a, b) => a - b);
      const maxLatency = Math.max(1, ...rows.map((r) => r.avg_latency || 0));
      gridHost.replaceChildren(txHeatmapSvg({ rows, agentIds, buckets, maxLatency }));
    } catch (err) {
      gridHost.replaceChildren(el('div', { class: 'error' }, errText(err)));
    }
  }
  bucketSel.addEventListener('change', loadHeatmap);
  await loadHeatmap();
  return root;
}
...
async function txTrendView() {
  const tests = await api('/api/transaction-tests');
  if (!tests.length) return el('div', { class: 'empty' }, 'Ingen transaktionstests endnu.');

  const selId = txDetailId || tests[0].id;
  const test  = tests.find((t) => t.id === selId) || tests[0];
  const testSel = el('select', {}, ...tests.map((t) => el('option', { value: String(t.id), ...(t.id === selId ? { selected: 'selected' } : {}) }, t.name)));
  testSel.addEventListener('change', () => { txDetailId = Number(testSel.value); render(); });
  const daysSel = el('select', {}, el('option', { value: '7' }, '7 dage'), el('option', { value: '30' }, '30 dage'));

  const root = el('div');
  const chartHost  = el('div', { style: 'margin-top:12px' });
  const summaryHost = el('div', { style: 'margin-top:8px' });
  const resultsHost = el('div', { style: 'margin-top:16px' });

  root.append(el('div', { class: 'history-controls' },
    el('label', { class: 'inline muted' }, 'Test ', testSel),
    el('label', { class: 'inline muted' }, 'Periode ', daysSel),
  ), chartHost, summaryHost, resultsHost);

  async function loadTrend() {
    try {
      const [trendData, resultsData] = await Promise.all([
        api(`/api/transaction-tests/${selId}/trend?days=${daysSel.value}`),
        api(`/api/transaction-tests/${selId}/results?limit=10`),
      ]);

      // Aggregate per-step medians by day
      const stepNames = (test.steps || []).map((s, i) => s.name || `Trin ${i + 1}`);
      const byStepDay = {};
      for (const row of trendData.rows) {
        for (let si = 0; si < row.steps.length; si++) {
          const key = `${si}_${row.day}`;
          if (!byStepDay[key]) byStepDay[key] = [];
          if (row.steps[si] && row.steps[si].duration_ms != null) byStepDay[key].push(row.steps[si].duration_ms);
        }
      }
      const allDays = [...new Set(trendData.rows.map((r) => String(r.day)))].sort();
      const stepSeries = stepNames.map((name, si) => {
        const points = allDays.map((day) => {
          const vals = byStepDay[`${si}_${day}`] || [];
          const sorted = vals.slice().sort((a, b) => a - b);
          const median = sorted.length ? sorted[Math.floor(sorted.length / 2)] : null;
          return { day, median };
        }).filter((p) => p.median != null);
        const allMedians = points.map((p) => p.median);
        const baseline = allMedians.length ? allMedians[Math.floor(allMedians.length / 2)] : null;
        return { name, points, baseline };
      });

      chartHost.replaceChildren(txTrendSvg({ stepSeries, days: Number(daysSel.value), testSteps: test.steps || [] }));

      // Text summary lines
      const lines = stepSeries.map((s) => {
        if (!s.points.length) return null;
        const latest = s.points[s.points.length - 1].median;
        const pct = s.baseline ? Math.round((latest - s.baseline) / s.baseline * 100) : null;
        const suffix = pct != null && Math.abs(pct) >= 10 ? ` · ${pct > 0 ? '+' : ''}${pct}% over baseline` : '';
        return el('div', { class: 'muted small' }, `${s.name}: ${Math.round(latest)} ms median${suffix}`);
      }).filter(Boolean);
      summaryHost.replaceChildren(...lines);

      // Recent failures diagnosis
      const failures = (resultsData.results || []).filter((r) => r.status !== 'ok');
      if (failures.length) {
        const failEls = failures.slice(0, 5).map((r) => {
          const stepIdx = r.error_detail && r.error_detail.step != null ? r.error_detail.step : null;
          const diag = txDiagnose(r.error_detail, stepIdx);
          return el('div', { class: 'probe-detail-row' },
            txStatusBadge(r.status),
            el('span', { class: 'muted small', style: 'margin-left:6px' }, fmtDate(r.ran_at)),
            diag ? el('div', { class: 'small', style: 'margin-top:2px' }, diag) : null,
          );
        });
        resultsHost.replaceChildren(
          el('div', { class: 'muted small', style: 'margin-bottom:4px' }, 'Seneste fejl'),
          ...failEls,
        );
      } else {
        resultsHost.replaceChildren();
      ...
      chartHost.replaceChildren(el('div', { class: 'error' }, errText(err)));
    }
  }
  daysSel.addEventListener('change', loadTrend);
  await loadTrend();
  return root;
}
...
function txEditModal(test, agents) {
  const card = $('#modal-card');
  const isEdit = !!test;
  const data = test || { name: '', type: 'http', steps: [], secret_names: [], agents: ['all'], enabled: true };

  const nameInput = el('input', { type: 'text', value: data.name, placeholder: 'e.g. Login → dashboard' });
  const enabledChk = el('input', { type: 'checkbox', ...(data.enabled ? { checked: 'checked' } : {}) });

  // Agent assignment
  const agentMode = data.agents && data.agents[0] !== 'all' ? 'specific' : 'all';
  const agentModeSel = el('select', {},
    el('option', { value: 'all' }, 'Alle agenter'),
    el('option', { value: 'specific', ...(agentMode === 'specific' ? { selected: 'selected' } : {}) }, 'Specifikke agenter'),
  );
  const selectedAgents = Array.isArray(data.agents) && data.agents[0] !== 'all' ? data.agents : [];
  const agentsBox = el('div', { class: 'check-list' },
    ...agents.map((a) => checkRow(a.id, a.display_name || a.hostname, selectedAgents.includes(a.id))));
  const agentsWrap = el('label', {}, 'Agenter', agentsBox);
  const syncAgentMode = () => { agentsWrap.style.display = agentModeSel.value === 'specific' ? '' : 'none'; };
  agentModeSel.addEventListener('change', syncAgentMode);

  // Secrets panel — write-only, never shows stored values
  const secretsBox = el('div', { class: 'tc-list' });
  const secretRows = []; // [{nameInput, valueInput}]
  // Show existing secret names as "sat" indicators
  for (const name of (data.secret_names || [])) {
    addSecretRow(name, null, true);
  }
  function addSecretRow(existingName, existingValue, isExisting) {
    const nameIn = el('input', { type: 'text', value: existingName || '', placeholder: 'Navn (f.eks. API_KEY)' });
    const valIn  = el('input', { type: 'password', placeholder: isExisting ? '(sat — skriv for at ændre)' : 'Værdi' });
    if (existingValue) valIn.value = existingValue;
    const del = el('button', { type: 'button', class: 'small ghost danger', onclick: () => { const i = secretRows.findIndex((r) => r.nameIn === nameIn); if (i >= 0) secretRows.splice(i, 1); row.remove(); } }, '×');
    const row = el('div', { class: 'test-item-row' }, nameIn, valIn, del);
    secretRows.push({ nameIn, valIn, isExisting });
    secretsBox.append(row);
  }
  const addSecretBtn = el('button', { type: 'button', class: 'small ghost', onclick: () => addSecretRow('', '', false) }, '+ Tilføj hemmelighed');

  // Steps editor
  const stepsBox = el('div', { class: 'tc-list' });
  const stepRows = [];
  function addStepRow(step) {
    const nameIn    = el('input', { type: 'text', value: step ? step.name || '' : '', placeholder: 'Navn' });
    const methodSel = el('select', {}, ...['GET', 'POST', 'PUT', 'PATCH', 'DELETE'].map((m) => el('option', { value: m, ...(step && step.method === m ? { selected: 'selected' } : {}) }, m)));
    const urlIn     = el('input', { type: 'text', value: step ? step.url || '' : '', placeholder: 'https://…', style: 'flex:2' });
    const headersIn = el('input', { type: 'text', value: step && step.headers ? JSON.stringify(step.headers) : '', placeholder: 'Headers JSON ({"X-Token":"{{TOKEN"}})' });
    const bodyIn    = el('textarea', { rows: '2', placeholder: 'Body (JSON/form/text)' }); if (step && step.body) bodyIn.value = step.body;
    const statusIn  = el('input', { type: 'number', min: '100', max: '599', value: step && step.expect_status ? step.expect_status : '', placeholder: 'HTTP kode', style: 'width:7em' });
    const keyIn     = el('input', { type: 'text', value: step && step.expect_keyword ? step.expect_keyword : '', placeholder: 'Forventet ord i svar' });
    const extractIn = el('input', { type: 'text', value: step && Array.isArray(step.extract) ? step.extract.map((e) => `${e.name}=${e.regex}`).join(', ') : '', placeholder: 'Udtræk: TOKEN="bearer (\\w+)"' });
    const ctrl = { nameIn, methodSel, urlIn, headersIn, bodyIn, statusIn, keyIn, extractIn };
    const del = el('button', { type: 'button', class: 'small ghost danger', title: 'Fjern trin', onclick: () => { const i = stepRows.indexOf(ctrl); if (i >= 0) stepRows.splice(i, 1); node.remove(); } }, '×');
    const node = el('div', { class: 'test-item-row', style: 'flex-wrap:wrap;gap:4px' },
      el('div', { style: 'display:flex;gap:4px;width:100%' }, nameIn, methodSel, urlIn, del),
      el('div', { style: 'display:flex;gap:4px;width:100%;flex-wrap:wrap' }, headersIn, statusIn, keyIn),
      el('div', { style: 'display:flex;gap:4px;width:100%' }, bodyIn),
      el('div', { style: 'display:flex;gap:4px;width:100%' }, extractIn),
    );
    stepRows.push(ctrl);
    stepsBox.append(node);
    return ctrl;
  }
  for (const s of (data.steps || [])) addStepRow(s);
  const addStepBtn = el('button', { type: 'button', class: 'small ghost', onclick: () => addStepRow(null) }, '+ Tilføj trin');

  const err = el('p', { class: 'error' });
  const saveBtn = el('button', { type: 'button', class: 'small' }, isEdit ? 'Gem ændringer' : 'Opret');

  function collectSteps() {
    return stepRows.map((c) => {
      const step = {
        name: c.nameIn.value.trim() || 'Unavngivet trin',
        method: c.methodSel.value,
        url: c.urlIn.value.trim(),
      };
      const h = c.headersIn.value.trim();
      if (h) { try { step.headers = JSON.parse(h); } catch { /* ignore */ } }
      const b = c.bodyIn.value.trim(); if (b) step.body = b;
      const s = c.statusIn.value; if (s) step.expect_status = Number(s);
      const k = c.keyIn.value.trim(); if (k) step.expect_keyword = k;
      const ex = c.extractIn.value.trim();
      if (ex) {
        step.extract = ex.split(',').map((part) => {
          const eq = part.indexOf('=');
          return eq > 0 ? { name: part.slice(0, eq).trim(), regex: part.slice(eq + 1).trim() } : { name: part.trim(), regex: '' };
        });
      }
      return step;
    });
  ...
  function collectSecrets() {
    const out = {};
    for (const { nameIn, valIn, isExisting } of secretRows) {
      const n = nameIn.value.trim(); if (!n) continue;
      // Only include if a value is provided; existing secrets with no new value
      // are preserved server-side when the secrets field is omitted.
      if (valIn.value) out[n] = valIn.value;
    }
    return out;
  }
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    if (!name) { err.textContent = 'Navn er påkrævet.'; return; }
    const agts = agentModeSel.value === 'specific' ? checkedValues(agentsBox) : ['all'];
    if (agentModeSel.value === 'specific' && !agts.length) { err.textContent = 'Vælg mindst én agent.'; return; }
    const body = {
      name,
      type: 'http',
      steps: collectSteps(),
    ...
    saveBtn.disabled = true;
    try {
      if (isEdit) await api(`/api/transaction-tests/${test.id}`, { method: 'PUT', body });
      else await api('/api/transaction-tests', { method: 'POST', body });
      toast(isEdit ? 'Gemt' : 'Oprettet');
      closeModal();
      render();
    } catch (e2) { err.textContent = errText(e2); saveBtn.disabled = false; }
  });
  ...
  const form = el('div', { class: 'form-grid test-form' },
    el('label', {}, 'Navn', nameInput),
    el('label', { class: 'inline' }, enabledChk, ' Aktiv'),
    el('label', {}, 'Agenter', agentModeSel),
    agentsWrap,
    el('div', {},
      el('div', { class: 'muted small', style: 'margin-bottom:4px' }, 'Hemmeligheder (skrives kun — vises aldrig)'),
      secretsBox,
      el('div', { class: 'form-actions' }, addSecretBtn)),
    el('div', {},
      el('div', { class: 'muted small', style: 'margin-bottom:4px' }, 'Trin (køres i rækkefølge — stop ved første fejl)'),
      stepsBox,
      el('div', { class: 'form-actions' }, addStepBtn)),
    err,
    el('div', { class: 'form-actions' },
      el('button', { type: 'button', class: 'ghost', onclick: closeModal }, 'Annuller'),
      saveBtn));
  syncAgentMode();
  card.replaceChildren(el('h3', {}, isEdit ? 'Rediger transaktionstest' : 'Ny transaktionstest'), form);
  card.classList.add('wide');
  $('#modal').classList.remove('hidden');
}
...
PAGE_INFO.transactionTests = {
  hero: 'Transaktionstests — multi-trin HTTP-sekvenser der køres fra agenter og giver latenstid, fejldiagnose og trendanalyse pr. trin.',
  title: 'Transaktionstests',
  body: () => [
    el('p', {}, 'En transaktionstest er en navngivet sekvens af HTTP-trin (GET/POST/…) der køres i rækkefølge fra udvalgte agenter. Hvert trin kan validere HTTP-statuskode og et nøgleord i svaret samt udtrække værdier til brug i efterfølgende trin (f.eks. et session-token).'),
    el('h4', {}, 'Liste'),
    el('p', {}, 'Oversigt over alle konfigurerede tests. Klik ', el('strong', {}, 'Rediger'), ' (kræver admin) for at ændre trin, agenter og hemmeligheder. Hemmeligheder skrives kun — de lagres på serveren og vises aldrig igen, kun nøglenavnet ("sat / ikke sat").'),
    el('h4', {}, 'Matrix'),
    el('p', {}, 'Agenter × tests som en farvet tabel. Grøn = OK, gul = fejl, rød = kritisk. En pil (↑/↓) angiver at seneste kørsel var mere end 10 % langsommere/hurtigere end medianen.'),
    el('h4', {}, 'Tidsheatmap'),
    el('p', {}, 'SVG-grid: X = tidsbuckets (5 min / 15 min / 1 time), Y = agenter. Farven afspejler gennemsnitlig latenstid (grøn→gul→rød); mørke celler angiver fejl. Tooltip: avg latenstid, fejl, antal kørsler.'),
    el('h4', {}, 'Trend pr. trin'),
    el('p', {}, 'Kurve pr. trin over de seneste 7 eller 30 dage. Stiplet reference-linje = baseline-median. Tekstlinje opsummerer afvigelse. Seneste fejl vises med en læsbar diagnose baseret på fejlfase og fejlkode.'),
  ],
};
