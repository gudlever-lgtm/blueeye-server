...
| 052 | (retire) | **retire Enterprise + HA** — drops the `ha_nodes` table (HA removed) and reconciles `license_plans`/`licenses` after the plan catalogue collapsed to Pilot/Starter/Professional (Enterprise + MSP retired) |
...
| CMDB integration (single source of truth) | migration `051_create_cmdb.sql` (`cmdb_config` singleton + `agent_cmdb_links`); repos `src/repositories/cmdbConfigRepository.js` + `agentCmdbLinksRepository.js`; validation `src/validation/cmdbValidation.js`; routes `src/routes/cmdb.js` — admin config + connection test (`/api/settings/cmdb`, `/test`), operator+ asset search (`/api/cmdb/assets/search`), and per-agent links (`/api/agents/:id/cmdb-link`). REUSES the integrations connector registry (ServiceNow/Nautobot `testConnection()`/`search()`) + `secretBox`. Credentials encrypted at rest, never returned. Linking an asset **syncs `agents.location_id`** (match-or-create a location by the asset's CMDB location name). Also a target in the **Test area** (`screenCmdb` + `cmdb` in `src/routes/diagnostics.js`). **UI: Settings → CMDB** (`settingsCmdbView`) + per-agent card (`loadAgentCmdbLink`, debounced search + removable chip). Docs: `docs/cmdb.md` |
