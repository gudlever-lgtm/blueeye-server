...
| Documentation / how-to content (Documentation tab) | `views.docs` + the `DOCS` content array + `docs*` helpers in `public/app.js` (static, no backend); nav button (`data-view="docs"`) in `public/index.html`; `PAGE_INFO.docs`; `.docs-*` CSS. RBAC: the `Administration & setup` section carries `admin:true` and is dropped for non-admins. Prose guide in `docs/documentation-center.md` |
