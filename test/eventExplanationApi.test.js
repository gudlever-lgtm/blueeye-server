...
async function seed() {
  ...
  await findingStore.save({ id: 'pf', hostId: '7', metric: 'io.await', kind: 'ANOMALY', severity: 'CRIT', observed: 40, baseline: 5, deviation: 6.2, explanation: 'io.await at 40 deviated 6.2σ', evidence: [{ metric: 'io.await', value: 40, ts: '2026-06-01T00:00:00Z' }], eventCaseId: id, createdAt: new Date('2026-06-01T00:00:00Z') });
  const agentsRepo = makeAgentsRepo({ findById: async (aid) => (Number(aid) === 7 ? { id: 7, display_name: 'core-sw-1', hostname: 'h7' } : null) });
  return { eventCasesRepo, findingStore, agentsRepo, id };
}

test('GET /api/events/:id includes a separate explanation (what/where/why) → 200', async () => {
  const { eventCasesRepo, findingStore, agentsRepo, id } = await seed();
...
test('GET /api/events/:id explanation.why is the raw fallback when there is no primary finding', async () => {
  const eventCasesRepo = makeEventCasesRepo();
  const id = await eventCasesRepo.create({ host_id: '9', title: 'bare', status: 'open', severity: 'WARN', first_event_at: new Date(), last_event_at: new Date() });
  const res = await request(makeApp({ eventCasesRepo })).get(`/api/events/${id}`).set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 200);
