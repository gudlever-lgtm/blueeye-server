...
(function (root) {
  ...
  var RELEASES = [
    { v: '0.176.0', d: '2026-09-20', area: 'platform',
      en: { t: 'A switch has its own page, and its own credentials', s: 'Every polled switch now has a page: the ports, what each one is carrying, and a chart of any port’s history. Credentials moved off the individual device and into profiles — one per site, one global default, an override per switch where you need it — and SNMPv3 works, with the keys encrypted at rest and never returned by the API. The server resolves which credential a switch should use and sends exactly that one, rather than having the agent try several in turn: trying credentials in sequence is indistinguishable from an attack, and it locks SNMPv3 accounts.' },
      da: { t: 'En switch har sin egen side — og sine egne logins', s: 'Hver pollet switch har nu en side: portene, hvad hver enkelt bærer, og en graf over en ports historik. Adgangskoder er flyttet fra den enkelte enhed over i profiler — én pr. lokation, én global standard, og en undtagelse pr. switch hvor der er brug for det — og SNMPv3 virker, med nøglerne krypteret og uden at API’et nogensinde giver dem tilbage. Serveren finder ud af hvilket login en switch skal bruge og sender præcis det ene, i stedet for at lade agenten prøve flere efter hinanden: at prøve logins i rækkefølge kan ikke skelnes fra et angreb, og det låser SNMPv3-konti.' } },
    { v: '0.175.0', d: '2026-09-20', area: 'diagnostics',
      en: { t: 'A network loop now names the two cables', s: 'A forwarding loop makes everything slow at once, in waves, and the cause is one cable somewhere. BlueEyes now recognises the signature — the same MAC address bouncing between two switch ports over and over — and says which two ports it is bouncing between, so the search is two cables instead of a building. Broadcast traffic rising on many ports at the same time and spanning tree reconverging repeatedly are read as corroboration, never on their own: a MAC moves when somebody unplugs a laptop, and a detector that fires on that is one nobody keeps switched on.' },
      da: { t: 'Et netværksloop udpeger nu de to kabler', s: 'Et forwarding-loop gør alt langsomt på én gang, i bølger, og årsagen er ét kabel et sted. BlueEyes genkender nu signaturen — den samme MAC-adresse der hopper frem og tilbage mellem to switchporte — og siger hvilke to porte den hopper imellem, så søgningen er to kabler i stedet for en bygning. Broadcast-trafik der stiger på mange porte samtidig og spanning tree der omlægger gentagne gange læses som bekræftelse, aldrig alene: en MAC flytter sig når nogen trækker en bærbar ud, og en detektor der udløser på det er en ingen lader være tændt.' } },
    { v: '0.174.0', d: '2026-09-20', area: 'insights',
      en: { t: 'A finding can point at a switch port', s: 'Anomaly detection used to stop at the agent: a finding named a host, so a switch port quietly dropping frames had nowhere to appear. Findings now carry the device and the port as well, and per-interface errors, discards and utilisation go through the same median-and-deviation detection as everything else — with the port in the metric name, so one flapping port cannot raise the baseline for the other forty-seven. A port that has never discarded and starts discarding four a second is now something the product notices on its own.' },
      da: { t: 'Et fund kan pege på en switchport', s: 'Afvigelsesdetektionen stoppede før ved agenten: et fund navngav en vært, så en switchport der stille tabte frames havde ingen steder at dukke op. Fund bærer nu også enheden og porten, og fejl, discards og udnyttelse pr. interface går gennem samme median-og-afvigelse-detektion som alt andet — med porten i metriknavnet, så én dårlig port ikke kan hæve baseline for de andre syvogfyrre. En port der aldrig har tabt noget og pludselig taber fire i sekundet er nu noget produktet selv opdager.' } },
