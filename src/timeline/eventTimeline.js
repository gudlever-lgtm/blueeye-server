...
// Pure read-model for the cross-agent EVENT-CLUSTER timeline (Fase 2 —
// "Event Situation View"). Merges rows already read from existing sources
// into ONE chronological event stream spanning the cluster's affected agents,
// and splits out the "what changed just before" pre-event events.
//
// It builds on the per-target timeline (targetTimeline.js): the finding /
// event / agent-event / playbook-run mappers are reused verbatim, then each
...
//
// No storage, no I/O: the service (eventTimelineService.js) fetches the rows
// and hands them here.
//
// Normalised event shape:
//   { timestamp, source, target, type, severity, summary, ref_id }
//     source — finding | event | agent | playbook | config | status
...
// the "what changed" panel surfaces when they land in the pre-event window.
const PRE_EVENT_SOURCES = new Set([
  CLUSTER_SOURCES.PLAYBOOK, CLUSTER_SOURCES.AGENT, CLUSTER_SOURCES.CONFIG,
]);

const SOURCE_ORDER = { finding: 0, event: 1, agent: 2, playbook: 3, config: 4, status: 5, verification: 6, evidence: 7 };
...
// 'event'): open→acknowledged, →resolved, etc. This is the state-transition
...
// Build the merged cluster timeline + the "what changed" pre-event slice.
//
//   memberFindings — the cluster's member finding objects (source a). Each is
//                    tagged with its own hostId as the target.
//   agentSources   — [{ agentId, agentEvents, playbookRuns, events,
//                    configChanges }] per affected agent.
//   statusChanges  — the cluster's lifecycle transitions (source b).
//   firstFindingAt — the cluster's earliest member finding time (the event
//                    onset); the "what changed" window ends here.
//   lookbackMs     — how far before firstFindingAt the pre-event window opens.
//
