...
// API tests for the event work log (Fase 3):
//   GET  /api/events/:id/notes   viewer+
//   POST /api/events/:id/notes   operator+
//
...
// An event_cases fake seeded with one open event (id 1).
function seededCases() {
  const repo = makeEventCasesRepo();
  repo.rows.push({
    id: 1,
    host_id: 'agent-7',
...
function appWith(overrides = {}) {
  return makeApp({ eventCasesRepo: seededCases(), ...overrides });
}

const post = (app, role, body, id = 1) =>
  request(app).post(`/api/events/${id}/notes`).set('Authorization', authHeader(role)).send(body);

const get = (app, role, id = 1) =>
  request(app).get(`/api/events/${id}/notes`).set('Authorization', authHeader(role));
...
test('POST /notes returns 201 for each valid kind', async () => {
  for (const kind of ['observation', 'action', 'ruled_out']) {
    ...
    assert.equal(res.body.note.eventCaseId, 1);
  }
});
...
test('POST /notes returns 400 for a non-numeric event id', async () => {
  const res = await request(appWith())
    .post('/api/events/abc/notes')
...
test('POST /notes returns 401 without a token', async () => {
  const res = await request(appWith()).post('/api/events/1/notes').send({ text: 'x', kind: 'action' });
...
test('a rejected viewer write leaves the log untouched', async () => {
  ...
  const app = appWith({ eventNotesRepo });
  await post(app, 'viewer', { text: 'should not land', kind: 'observation' });
  assert.equal(eventNotesRepo.rows.length, 0);
});

// ------------------------------------------------------------------- POST 404
test('POST /notes returns 404 for an unknown event id', async () => {
  const res = await post(appWith(), 'operator', { text: 'x', kind: 'action' }, 9999);
  assert.equal(res.status, 404);
  assert.equal(res.body.error, 'Event not found');
});

test('POST /notes validates the body before checking the event exists', async () => {
  // A bad body is the caller's fault regardless of which event they meant;
...
test('POST /notes returns 500 (clean, no stack) when the repo throws', async () => {
  const eventNotesRepo = makeEventNotesRepo({ append: throwingAsync() });
  const res = await post(appWith({ eventNotesRepo }), 'operator', { text: 'x', kind: 'action' });
...
test('GET /notes returns an empty log (not 404) for an event with no entries', async () => {
...
test('GET /notes returns 401 without a token', async () => {
  assert.equal((await request(appWith()).get('/api/events/1/notes')).status, 401);
});

test('GET /notes returns 404 for an unknown event and 400 for a bad id', async () => {
  assert.equal((await get(appWith(), 'viewer', 9999)).status, 404);
  const bad = await request(appWith()).get('/api/events/abc/notes').set('Authorization', authHeader('viewer'));
  assert.equal(bad.status, 400);
});

test('GET /notes returns 500 (clean) when the repo throws', async () => {
  const eventNotesRepo = makeEventNotesRepo({ listForEvent: throwingAsync() });
  const res = await get(appWith({ eventNotesRepo }), 'viewer');
  assert.equal(res.status, 500);
  assert.equal(res.body.error, 'Internal Server Error');
  assert.ok(!/at Object|at async|\.js:\d+:\d+/.test(JSON.stringify(res.body)), 'must not leak a stack trace');
});
...
test('GET /notes returns 500 when the ruled-out query alone fails', async () => {
  ...
  const eventNotesRepo = makeEventNotesRepo({ listRuledOut: throwingAsync() });
  const res = await get(appWith({ eventNotesRepo }), 'viewer');
  assert.equal(res.status, 500);
  assert.equal(res.body.error, 'Internal Server Error');
});
...
test('the work log exposes no edit or delete route', async () => {
  ...
  for (const [method, path] of [
    ['patch', `/api/events/1/notes/${noteId}`],
    ['put', `/api/events/1/notes/${noteId}`],
    ['delete', `/api/events/1/notes/${noteId}`],
    ['delete', '/api/events/1/notes'],
  ]) {
...
test('appending a note writes to the audit log without copying the note text', async () => {
  ...
  const entry = auditLogRepo.rows.find((r) => r.action === 'event_note_append');
  assert.ok(entry, 'expected an event_note_append audit entry');
  assert.equal(entry.category, 'event');
...
test('the hash chain stays verifiable after notes are appended', async () => {
  ...
  const noteRows = rows.filter((r) => r.action === 'event_note_append');
...
test('a failing audit logger does not break the note write', async () => {
  ...
  const eventNotesRepo = makeEventNotesRepo();
  const app = appWith({ auditLogRepo, auditLogger: createAuditLogger({ auditLogRepo }), eventNotesRepo });

  const res = await post(app, 'operator', { text: 'still recorded', kind: 'observation' });
  assert.equal(res.status, 201);
  assert.equal(eventNotesRepo.rows.length, 1);
});
...
test('the notes routes are absent when no notes repo is wired (pre-migration)', async () => {
  ...
  // rest of the event API rather than 500 on a missing table.
  const app = makeApp({ eventCasesRepo: seededCases(), eventNotesRepo: null });

  assert.equal((await get(app, 'viewer')).status, 404);
  assert.equal((await post(app, 'operator', { text: 'x', kind: 'action' })).status, 404);
  // The rest of the event API still works.
