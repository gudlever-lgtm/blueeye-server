...
-- Per-metric thresholds used to derive events from active-probe results.
...
CREATE TABLE IF NOT EXISTS probe_thresholds (
  ...
  CONSTRAINT fk_probe_thresholds_location FOREIGN KEY (location_id)
    REFERENCES locations (id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Events derived from probe_results (one per (agent, metric, target) outage).
-- started_at = first failing result in the breaching sequence; resolved_at NULL
-- while active. At most one active event per (agent, metric, target).
CREATE TABLE IF NOT EXISTS probe_outages (
  ...
  CONSTRAINT fk_probe_outages_agent FOREIGN KEY (agent_id)
    REFERENCES agents (id) ON DELETE CASCADE,
  CONSTRAINT fk_probe_outages_location FOREIGN KEY (location_id)
    REFERENCES locations (id) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS integration_audit (
  ...
  event VARCHAR(64) NOT NULL,                  -- 'event' | 'anomaly' | 'agent.enroll' | 'agent.delete' | 'test'
...
-- Security events (NIS2 sense — distinct from the network `events` table).
...
CREATE TABLE IF NOT EXISTS blueeye_nis2_reports (
  id INT UNSIGNED NOT NULL AUTO_INCREMENT,
  report_type ENUM('readiness', 'executive', 'risk', 'control', 'event') NOT NULL,
  title VARCHAR(255) NOT NULL,
...
-- control / risk / event / report. Stored as a reference, not a blob.
CREATE TABLE IF NOT EXISTS blueeye_nis2_evidence (
  ...
  entity_type ENUM('control', 'risk', 'event', 'report') NULL DEFAULT NULL,
...
-- risk, control or event; old_value/new_value hold JSON snapshots. Actor
...
CREATE TABLE IF NOT EXISTS blueeye_audit_log (
  ...
  entity_type VARCHAR(32) NOT NULL,               -- 'risk' | 'control' | 'event' | 'report'
  entity_id INT UNSIGNED NULL DEFAULT NULL,
...
-- Remediation playbooks + their per-event run history (migration 055). A
-- playbook is a pre-defined response keyed to an anomaly-type (trigger_condition
-- matches the event's primary finding metric, exactly); event_playbook_runs
-- records that a playbook ran against a specific event and the outcome.
...
CREATE TABLE IF NOT EXISTS event_playbook_runs (
  ...
  CONSTRAINT fk_event_playbook_runs_event FOREIGN KEY (event_case_id)
    REFERENCES event_cases (id) ON DELETE CASCADE,
  CONSTRAINT fk_event_playbook_runs_playbook FOREIGN KEY (playbook_id)
    REFERENCES remediation_playbooks (id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Cross-agent event clusters (migration 057): findings from DIFFERENT agents
