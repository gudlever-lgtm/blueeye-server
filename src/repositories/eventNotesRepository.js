...
// Data-access for `event_notes` (migration 072) — the event work log that
...
const BASE_COLUMNS = `id, event_case_id, kind, text, author_user_id,
...
function createEventNotesRepository(db) {
  const { pool } = db;
  ...
  async function append({
    ...
    const [res] = await pool.query(
      `INSERT INTO event_notes
         (event_case_id, kind, text, author_user_id, author_email, author_role)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [eventCaseId, kind, text, authorUserId, authorEmail, authorRole]
    );
  ...
  async function findById(id) {
    const [rows] = await pool.query(
      `SELECT ${BASE_COLUMNS} FROM event_notes WHERE id = ?`,
      [id]
    );
    return mapRow(rows[0]);
  }

  // The full log for one event, oldest-first. `limit` is a safety cap, not a
  ...
  async function listForEvent({ eventCaseId, limit = 500 }) {
    const [rows] = await pool.query(
      `SELECT ${BASE_COLUMNS} FROM event_notes
        WHERE event_case_id = ?
  ...
  // Only the ruled_out entries, oldest-first. Separate from listForEvent so
  ...
  async function listRuledOut({ eventCaseId, limit = 200 }) {
    const [rows] = await pool.query(
      `SELECT ${BASE_COLUMNS} FROM event_notes
        WHERE event_case_id = ? AND kind = 'ruled_out'
        ORDER BY created_at ASC, id ASC
        LIMIT ?`,
      [eventCaseId, limit]
    );
  ...
  async function countForEvent({ eventCaseId }) {
    const [rows] = await pool.query(
      'SELECT COUNT(*) AS n FROM event_notes WHERE event_case_id = ?',
      [eventCaseId]
    );
    return Number(rows[0] ? rows[0].n : 0);
  }

  return { append, findById, listForEvent, listRuledOut, countForEvent };
}

module.exports = { createEventNotesRepository, KINDS };
