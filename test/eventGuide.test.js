...
const { buildEventGuide, metricStep, primaryMetricOf } = require('../src/eventCases/guide');

const EVENT = { id: 3, deviceId: '9', primaryFindingId: 'a1', severity: 'CRIT', status: 'open' };

test('the guide always opens with a confirm-scope step and ends with document/escalate', () => {
  const g = buildEventGuide({ event: EVENT, anomalies: [{ id: 'a1', metric: 'cpu' }] });
  assert.equal(g.steps[0].title, 'Confirm the event is still active');
...
test('a similar past event adds a resolution step that deep-links to it', () => {
  const g = buildEventGuide({
    event: EVENT,
    anomalies: [{ id: 'a1', metric: 'cpu' }],
    similar: [{ id: 42, title: 'CPU on core', resolvedAt: '2026-05-01T00:00:00.000Z', closedBy: 'ops@x' }],
  });
  const sim = g.steps.find((s) => /similar event/i.test(s.title));
  assert.ok(sim);
...
test('multiple linked anomalies add a "follow the correlated anomalies" step', () => {
  const g = buildEventGuide({ event: EVENT, anomalies: [{ id: 'a1', metric: 'cpu' }, { id: 'a2', metric: 'latency' }] });
  const cor = g.steps.find((s) => /correlated anomalies/i.test(s.title));
  assert.ok(cor);
  assert.match(cor.detail, /2 anomalies fired together/);
});

test('empty event yields an empty guide', () => {
  assert.deepEqual(buildEventGuide({}), { eventId: null, primaryMetric: null, steps: [] });
});
