...
const { buildTimeline, statusFromDetail } = require('../src/eventCases/timeline');
...
const {
  makeApp, makeEventCasesRepo, makeFindingStore, makeAuditEventsRepo, makeAuditLogRepo, authHeader,
...
test('buildTimeline merges the sources into one chronological flat list', () => {
  const events = buildTimeline({
    ...
    statusChanges: [
      { id: 9, created_at: '2026-06-01T08:15:00Z', action: 'event_status_change', detail: 'open→investigating', actor_email: 'op@x' },
    ],
  });
...
test('auditLog.listByTarget filters by target + category, oldest-first', async () => {
  const pool = fakePool((sql, params) => {
    ...
    assert.deepEqual(params.slice(0, 2), ['42', 'event']);
    return [[]];
  });
  const repo = createAuditLogRepository({ pool });
  assert.deepEqual(await repo.listByTarget({ category: 'event', target: 42 }), []);
});

// The rename moved this category from `incident` to `event`, but audit_log is
// hash-chained — the old rows CANNOT be rewritten. So the read has to match
// both, or every event opened before the upgrade loses its status history.
test('auditLog.listByTarget accepts several categories (reads pre-rename history)', async () => {
  const pool = fakePool((sql, params) => {
    assert.match(sql, /WHERE target = \? AND category IN \(\?, \?\)/);
    assert.deepEqual(params.slice(0, 3), ['42', 'event', 'incident']);
    return [[]];
  });
  const repo = createAuditLogRepository({ pool });
  assert.deepEqual(await repo.listByTarget({ category: ['event', 'incident'], target: 42 }), []);
});
...
async function seedEvent({ status = 'open' } = {}) {
  ...
  return { eventCasesRepo, id };
}

test('GET timeline for an empty event returns 200 with no events', async () => {
  const { eventCasesRepo, id } = await seedEvent();
  const app = makeApp({ eventCasesRepo, findingStore: makeFindingStore() });
  const res = await request(app).get(`/api/events/${id}/timeline`).set('Authorization', authHeader('viewer'));
...
test('GET timeline aggregates anomalies + config-changes + status changes → 200', async () => {
  ...
  await auditLogRepo.record({ category: 'event', action: 'event_status_change', target: String(id), detail: 'open→investigating', actorEmail: 'op@x' });

  const app = makeApp({ eventCasesRepo, findingStore, auditEventsRepo, auditLogRepo });
  const res = await request(app).get(`/api/events/${id}/timeline`).set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 200);
...
test('GET timeline still shows status changes audited BEFORE the rename', async () => {
  // Rows written by the previous release carry category `incident`. audit_log is
  // hash-chained, so they cannot be rewritten to `event` — an upgraded server
  // must read them as they are, or an event's whole history silently vanishes.
  const { eventCasesRepo, id } = await seedEvent();
  const auditLogRepo = makeAuditLogRepo();
  ...
  const app = makeApp({ eventCasesRepo, findingStore: makeFindingStore(), auditLogRepo });
  const res = await request(app).get(`/api/events/${id}/timeline`).set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 200);
  const st = res.body.events.find((e) => e.type === 'status_change');
  assert.ok(st, 'a pre-rename status change must still appear on the timeline');
  assert.equal(st.status, 'investigating');
});

test('GET timeline only correlates config-changes on the SAME device', async () => {
  const { eventCasesRepo, id } = await seedEvent(); // host_id '9'
  const auditEventsRepo = makeAuditEventsRepo();
  await auditEventsRepo.record({ actorType: 'user', action: 'agent.update', targetType: 'agent', targetId: '7' }); // other device
  const app = makeApp({ eventCasesRepo, findingStore: makeFindingStore(), auditEventsRepo });
  const res = await request(app).get(`/api/events/${id}/timeline`).set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 200);
  assert.deepEqual(res.body.events, []); // the config change on agent 7 is not included
});

test('GET timeline is 404 for an unknown event', async () => {
  const app = makeApp();
  const res = await request(app).get('/api/events/9999/timeline').set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 404);
});

test('GET timeline is 400 for a non-numeric id', async () => {
  const app = makeApp();
  const res = await request(app).get('/api/events/abc/timeline').set('Authorization', authHeader('viewer'));
...
test('GET timeline surfaces a repo failure as 500', async () => {
  ...
  const res = await request(app).get('/api/events/1/timeline').set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 500);
});

test('GET timeline requires auth → 401', async () => {
  const app = makeApp();
  assert.equal((await request(app).get('/api/events/1/timeline')).status, 401);
});
