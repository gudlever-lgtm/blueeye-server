...
const {
  makeApp, makeEventClustersRepo, makeFindingStore, makeEvidenceSnapshotsRepo,
...
async function withEvidence(over = {}) {
  ...
  const app = makeApp({ eventClustersRepo, findingStore, evidenceRepo, snapshotService, auditLogRepo });
  return { app, eventClustersRepo, findingStore, evidenceRepo, auditLogRepo, snapshotService, captures, id, members };
}

// ---- GET /api/event-clusters/:id/evidence ------------------------------
...
test('GET /:id/evidence lists snapshots for the cluster (viewer+) → 200', async () => {
  ...
  const res = await request(app).get(`/api/event-clusters/${id}/evidence`).set('Authorization', authHeader('viewer'));
...
test('GET /:id/evidence requires auth → 401', async () => {
  ...
  assert.equal((await request(app).get(`/api/event-clusters/${id}/evidence`)).status, 401);
});

test('GET /:id/evidence is 404 for an unknown cluster', async () => {
  const { app } = await withEvidence();
  const res = await request(app).get('/api/event-clusters/99999/evidence').set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 404);
});

test('GET /:id/evidence is 400 for a non-numeric id', async () => {
  const { app } = await withEvidence();
  const res = await request(app).get('/api/event-clusters/abc/evidence').set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 400);
});

// ---- GET /api/event-clusters/:id/evidence/:sid -------------------------
...
test('GET /:id/evidence/:sid returns the raw-text payload (text/plain) → 200', async () => {
  ...
  await evidenceRepo.complete(sid, { status: 'complete', items: [{ name: 'agent.state', status: 'ok' }], payloadText: '# agent.state [ok]\nconnected: yes' });
  const { app, id } = await withEvidence({ evidenceRepo });
  const res = await request(app).get(`/api/event-clusters/${id}/evidence/${sid}`).set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 200);
...
// ---- POST /api/event-clusters/:id/evidence (manual re-snapshot) ---------
...
test('POST /:id/evidence triggers a manual capture of the cluster targets (operator+) → 202', async () => {
  ...
  const res = await request(app).post(`/api/event-clusters/${id}/evidence`).set('Authorization', authHeader('operator'));
...
test('POST /:id/evidence is forbidden for a viewer → 403', async () => {
  const { app, id } = await withEvidence();
  const res = await request(app).post(`/api/event-clusters/${id}/evidence`).set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 403);
});

test('POST /:id/evidence requires auth → 401', async () => {
  const { app, id } = await withEvidence();
  assert.equal((await request(app).post(`/api/event-clusters/${id}/evidence`)).status, 401);
});

test('POST /:id/evidence is 404 for an unknown cluster', async () => {
  const { app } = await withEvidence();
  const res = await request(app).post('/api/event-clusters/99999/evidence').set('Authorization', authHeader('operator'));
  assert.equal(res.status, 404);
});

test('POST /:id/evidence is rate-limited to once per window → 429 with Retry-After', async () => {
  const { app, id } = await withEvidence();
  const first = await request(app).post(`/api/event-clusters/${id}/evidence`).set('Authorization', authHeader('operator'));
  assert.equal(first.status, 202);
  const second = await request(app).post(`/api/event-clusters/${id}/evidence`).set('Authorization', authHeader('operator'));
