...
| Path visualization (traceroute map) | `src/analysis/pathGraph.js` (`buildPathGraph` — aggregates traceroutes → hop graph, geo/ASN, severity), `GET /api/probes/path`; UI `pathGraph()` in `public/app.js` (traceroute detail) — SVG topology **+ a geographic Leaflet map** (`drawPathMap`/`pathGeoStops`, source anchored at the agent site via `agentsRepo` `location_lat/lng`). Per-hop loss/jitter come from MTR-style agent traceroute. See `docs/path-visualization.md` |
