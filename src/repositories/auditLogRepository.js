...
//
// TAMPER-EVIDENCE (security pack): every row is chained to the previous one —
...
// verifyChain() walks the chain and reports the first broken row, if any. Rows
// written before migration 037 have NULL hashes and are simply skipped (the
// chain anchors at the first hashed row).
...
function createAuditLogRepository(db) {
  ...
  // to their column widths so an oversized detail never throws. Links the row
  // into the hash chain (prev_hash = last row's entry_hash).
  ...
  async function verifyChain({ limit = 100000 } = {}) {
    ...
    for (const row of rows) {
      const expected = entryHashFor(prev, row);
      if (row.prev_hash !== (prev || null) && (row.prev_hash || '') !== prev) {
