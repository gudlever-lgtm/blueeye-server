...
const { buildEventTimeline, PRE_EVENT_SOURCES } = require('../src/timeline/eventTimeline');
...
test('what-changed contains only sources c–e in [onset - lookback, onset)', () => {
  const { whatChanged } = buildEventTimeline({
  ...
  for (const e of whatChanged) {
    assert.ok(PRE_EVENT_SOURCES.has(e.source));
