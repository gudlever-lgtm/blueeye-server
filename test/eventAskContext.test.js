...
const {
  buildEventAskContext, gatherEventAskContext, maskIps, maskSecrets, maskConfigLine,
} = require('../src/eventCases/askContext');
const {
  makeEventCasesRepo, makeFindingStore, makeAuditEventsRepo, makeAuditLogRepo, makeConfigSnapshotsRepo,
...
// ---- buildEventAskContext (pure) ----------------------------------------

const EVENT = { id: 3, status: 'investigating', severity: 'CRIT', hostId: '9', title: 'CRIT cpu on 10.0.0.9', firstEventAt: '2026-06-01T08:00:00Z', lastEventAt: '2026-06-01T08:10:00Z', resolvedAt: null };

test('empty event yields empty context with hasAnyData=false', () => {
  const ctx = buildEventAskContext({ event: EVENT });
  assert.deepEqual(ctx.timeline, []);
  assert.deepEqual(ctx.configContext, []);
  assert.deepEqual(ctx.similarEvents, []);
  assert.equal(ctx.dataAvailability.hasAnyData, false);
  assert.deepEqual(ctx.dataAvailability, { timelineEvents: 0, configChanges: 0, similarEvents: 0, hasAnyData: false });
});

test('MASKING GUARANTEE: raw config text and secrets never appear in the context', () => {
  const ctx = buildEventAskContext({
    event: EVENT,
...
test('config changed lines are capped and stats preserved', () => {
  ...
  const ctx = buildEventAskContext({ event: EVENT, configDiffs: [{ stats: { added: 100, removed: 0 }, changedLines: many }] });
...
// ---- gatherEventAskContext (repo reads) ---------------------------------

test('gather returns null for an unknown event (route → 404)', async () => {
  const ctx = await gatherEventAskContext(999, { eventCasesRepo: makeEventCasesRepo(), findingStore: makeFindingStore() });
...
test('gather assembles timeline + config-context from all wired sources, masked', async () => {
  ...
  const id = await eventCasesRepo.create({ host_id: '9', title: 't', status: 'investigating', severity: 'CRIT', first_event_at: new Date('2026-06-01T08:00:00Z'), last_event_at: new Date('2026-06-01T08:10:00Z') });
  ...
  await auditLogRepo.record({ category: 'event', action: 'event_status_change', target: String(id), detail: 'open→investigating' });
  ...
  const ctx = await gatherEventAskContext(id, { eventCasesRepo, findingStore, auditEventsRepo, auditLogRepo, configSnapshotsRepo });

  assert.equal(ctx.event.id, id);
...
test('gather degrades gracefully with no config repo (similar always empty until Fase 4)', async () => {
  ...
  const ctx = await gatherEventAskContext(id, { eventCasesRepo, findingStore: makeFindingStore() });
  assert.deepEqual(ctx.configContext, []);
  assert.deepEqual(ctx.similarEvents, []);
});
