...
test('buildMatchingPlaybook: shows the run RESULT when it already ran on this event', () => {
...
test('buildHistoricalMatches: annotates timesSeen (pattern frequency) + playbook used', () => {
  ...
  const runsByEvent = { 1: [{ playbookId: 2, playbookName: 'PB', playbookActionType: 'run_probe', status: 'succeeded' }] };
  const out = buildHistoricalMatches(ranked, { runsByEvent, resolvedCandidates });
  assert.equal(out[0].timesSeen, 2); // two resolved cpu events
...
test('buildHistoricalMatches: playbook null when no run recorded', () => {
  ...
  const out = buildHistoricalMatches(ranked, { runsByEvent: {}, resolvedCandidates: [{ id: 1, primaryMetric: 'cpu' }] });
