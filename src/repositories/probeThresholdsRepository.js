...
// Data-access for `probe_thresholds`. Lookup precedence: a location-specific
// row wins over the global default (location_id IS NULL) for the same metric.
function createProbeThresholdsRepository(db) {
  const { pool } = db;
  ...
  async function getEffective(locationId, metric) {
    const [rows] = await pool.query(
      `SELECT ${COLUMNS} FROM probe_thresholds
  ...
  async function listGlobal() {
    const [rows] = await pool.query(
      `SELECT ${COLUMNS} FROM probe_thresholds WHERE location_id IS NULL ORDER BY metric`
    );
  ...
  async function listByLocation(locationId) {
    const [rows] = await pool.query(
      `SELECT ${COLUMNS} FROM probe_thresholds WHERE location_id = ? ORDER BY metric`,
      [locationId]
    );
    return rows.map(mapRow);
  }
  ...
  async function findById(id) {
    const [rows] = await pool.query(`SELECT ${COLUMNS} FROM probe_thresholds WHERE id = ?`, [id]);
    return mapRow(rows[0]) ?? null;
  }
  ...
  async function upsert({ location_id = null, metric, warning_value = null, critical_value = null, debounce_count = 3 }) {
    const [existing] = await pool.query(
      `SELECT id FROM probe_thresholds WHERE metric = ? AND location_id ${location_id == null ? 'IS NULL' : '= ?'}`,
      location_id == null ? [metric] : [metric, location_id]
    );
    if (existing[0]) {
      await pool.query(
        `UPDATE probe_thresholds SET warning_value = ?, critical_value = ?, debounce_count = ? WHERE id = ?`,
        [warning_value, critical_value, debounce_count, existing[0].id]
      );
      return findById(existing[0].id);
    }
    const [res] = await pool.query(
      `INSERT INTO probe_thresholds (location_id, metric, warning_value, critical_value, debounce_count)
       VALUES (?, ?, ?, ?, ?)`,
