...
const {
  makeApp, makeEventCasesRepo, makeFindingStore, makeAgentsRepo,
  makeRemediationPlaybooksRepo, authHeader,
} = require('../test-support/fakes');

// Seeds a target event (device 9, cpu anomaly) plus resolved/closed history,
...
async function seed({ withPlaybook = false } = {}) {
  ...
  const targetId = await eventCasesRepo.create({
  ...
  const aId = await eventCasesRepo.create({
  ...
  // The target's primary finding is linked to the event (so the masked AI
  // context has data when the AI path is exercised).
  await findingStore.save({ id: 'pf', hostId: '9', metric: 'cpu', severity: 'CRIT', kind: 'ANOMALY', explanation: 'x', observed: 95, baseline: 20, deviation: 6, evidence: [{ metric: 'cpu', value: 95, ts: new Date().toISOString() }], eventCaseId: targetId, createdAt: new Date('2026-06-01T08:00:00Z') });
  ...
  return { eventCasesRepo, findingStore, agentsRepo, remediationPlaybooksRepo, targetId, aId };
}

test('GET /recommendation returns the three ordered sections → 200 (viewer)', async () => {
  const { eventCasesRepo, findingStore, agentsRepo, remediationPlaybooksRepo, targetId } = await seed();
  const app = makeApp({ eventCasesRepo, findingStore, agentsRepo, remediationPlaybooksRepo });
  const res = await request(app).get(`/api/events/${targetId}/recommendation`).set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 200);
  // Section keys present, in order. `eventId` is the only id key — the
  // deprecated `incidentId` alias was dropped with the rest of the incident
  // vocabulary.
  assert.deepEqual(Object.keys(res.body), ['eventId', 'matching_playbook', 'historical_matches', 'ai_suggestion']);
...
test('GET /recommendation annotates resolution time + timesSeen + playbook used', async () => {
  const { eventCasesRepo, findingStore, agentsRepo, remediationPlaybooksRepo, targetId, aId } = await seed();
  // Record that a playbook ran (succeeded) on the resolved event A.
  const pbId = await remediationPlaybooksRepo.create({ name: 'PB-A', trigger_condition: 'cpu', action_type: 'run_probe' });
  await remediationPlaybooksRepo.recordRun({ eventCaseId: aId, playbookId: pbId, status: 'succeeded', resultText: 'fixed' });
  const app = makeApp({ eventCasesRepo, findingStore, agentsRepo, remediationPlaybooksRepo });
  const res = await request(app).get(`/api/events/${targetId}/recommendation`).set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 200);
  const a = res.body.historical_matches.find((h) => h.title === 'A');
  assert.equal(a.resolutionTimeSeconds, 3600); // 1 hour
  assert.equal(a.timesSeen, 1); // only A is a resolved cpu event
...
test('GET /recommendation matching_playbook suggests a matched playbook (manual)', async () => {
  const { eventCasesRepo, findingStore, agentsRepo, remediationPlaybooksRepo, targetId } = await seed({ withPlaybook: true });
  const app = makeApp({ eventCasesRepo, findingStore, agentsRepo, remediationPlaybooksRepo });
  const res = await request(app).get(`/api/events/${targetId}/recommendation`).set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 200);
...
test('GET /recommendation shows the run RESULT when the playbook already ran on this event', async () => {
  const { eventCasesRepo, findingStore, agentsRepo, remediationPlaybooksRepo, targetId } = await seed();
  ...
  await remediationPlaybooksRepo.recordRun({ eventCaseId: targetId, playbookId: pbId, status: 'failed', resultText: 'service would not restart', ranBy: 'op@x' });
  const app = makeApp({ eventCasesRepo, findingStore, agentsRepo, remediationPlaybooksRepo });
  const res = await request(app).get(`/api/events/${targetId}/recommendation`).set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 200);
...
test('GET /recommendation?force_ai=true is 403 for a viewer (operator+ only)', async () => {
  const { eventCasesRepo, findingStore, agentsRepo, remediationPlaybooksRepo, targetId } = await seed();
  const app = makeApp({ eventCasesRepo, findingStore, agentsRepo, remediationPlaybooksRepo });
  const res = await request(app).get(`/api/events/${targetId}/recommendation?force_ai=true`).set('Authorization', authHeader('viewer'));
...
test('GET /recommendation?force_ai=true is allowed for an operator → 200', async () => {
  const { eventCasesRepo, findingStore, agentsRepo, remediationPlaybooksRepo, targetId } = await seed();
  const app = makeApp({ eventCasesRepo, findingStore, agentsRepo, remediationPlaybooksRepo });
  const res = await request(app).get(`/api/events/${targetId}/recommendation?force_ai=true`).set('Authorization', authHeader('operator'));
  assert.equal(res.status, 200); // ai_suggestion still null until 1c is wired
});

test('GET /recommendation is 404 for an unknown event', async () => {
  const res = await request(makeApp()).get('/api/events/9999/recommendation').set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 404);
});

test('GET /recommendation is 400 for a non-numeric id', async () => {
  const res = await request(makeApp()).get('/api/events/abc/recommendation').set('Authorization', authHeader('viewer'));
...
test('GET /recommendation surfaces a repo failure as 500', async () => {
  ...
  const res = await request(makeApp({ eventCasesRepo })).get('/api/events/1/recommendation').set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 500);
});

test('GET /recommendation requires auth → 401', async () => {
  const res = await request(makeApp()).get('/api/events/1/recommendation');
...
// Builds an event that has NO matching playbook and NO resolved history, but a
// linked finding so the masked AI context has data (hasAnyData=true).
async function seedNoMatch() {
  const eventCasesRepo = makeEventCasesRepo();
  const id = await eventCasesRepo.create({ host_id: '5', title: 'lonely', status: 'open', severity: 'WARN', first_event_at: new Date('2026-06-01T00:00:00Z'), last_event_at: new Date('2026-06-01T00:00:00Z') });
  const findingStore = makeFindingStore();
  await findingStore.save({ id: 'f1', hostId: '5', metric: 'io.await', severity: 'WARN', kind: 'ANOMALY', explanation: 'x', evidence: [{ metric: 'io.await', value: 9, ts: new Date().toISOString() }], eventCaseId: id, createdAt: new Date('2026-06-01T00:00:00Z') });
  return { eventCasesRepo, findingStore, id };
}

test('ordering: no playbook + no history → the AI IS called, tagged ai_generated', async () => {
  const { eventCasesRepo, findingStore, id } = await seedNoMatch();
  const spy = spyAssistant();
  const app = makeApp({ eventCasesRepo, findingStore, assistant: spy.assistant });
  const res = await request(app).get(`/api/events/${id}/recommendation`).set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 200);
...
test('ordering: a matching playbook suppresses the AI call', async () => {
  const { eventCasesRepo, findingStore, agentsRepo, remediationPlaybooksRepo, targetId } = await seed({ withPlaybook: true });
  const spy = spyAssistant();
  const app = makeApp({ eventCasesRepo, findingStore, agentsRepo, remediationPlaybooksRepo, assistant: spy.assistant });
  const res = await request(app).get(`/api/events/${targetId}/recommendation`).set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 200);
...
test('ai_suggestion is the honest fallback (no provider call) when the context is empty', async () => {
  ...
  const eventCasesRepo = makeEventCasesRepo();
  const id = await eventCasesRepo.create({ host_id: '5', title: 'empty', status: 'open', severity: 'WARN', first_event_at: new Date(), last_event_at: new Date() });
  const spy = spyAssistant();
  const app = makeApp({ eventCasesRepo, assistant: spy.assistant });
  const res = await request(app).get(`/api/events/${id}/recommendation`).set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 200);
...
test('ai_suggestion is null when the assistant is disabled (endpoint still serves a+b)', async () => {
  ...
  const res = await request(makeApp({ eventCasesRepo, findingStore })).get(`/api/events/${id}/recommendation`).set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 200);
  assert.equal(res.body.ai_suggestion, null);
});
...
test('ai_suggestion is cached on the second identical request (one provider call)', async () => {
  ...
  const spy = spyAssistant();
  const app = makeApp({ eventCasesRepo, findingStore, assistant: spy.assistant });
  const first = await request(app).get(`/api/events/${id}/recommendation`).set('Authorization', authHeader('viewer'));
  const second = await request(app).get(`/api/events/${id}/recommendation`).set('Authorization', authHeader('viewer'));
