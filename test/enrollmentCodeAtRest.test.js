...
test('findById yields null when there is nothing decryptable, and never reads a cleartext column', async () => {
  ...
  // A row minted before migration 072: the migration dropped the cleartext, so
...
test('migration 072 backfills the hash before dropping the cleartext column', () => {
  const sql = fs.readFileSync(path.join(__dirname, '..', 'migrations', '072_enrollment_code_at_rest.sql'), 'utf8');
