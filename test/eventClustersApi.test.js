...
const { makeApp, makeEventClustersRepo, makeFindingStore, authHeader } = require('../test-support/fakes');
...
async function withCluster(over = {}) {
  ...
  const app = makeApp({ eventClustersRepo, findingStore });
  return { app, eventClustersRepo, findingStore, id, members };
}

// ---- GET /api/event-clusters -------------------------------------------

test('GET list returns clusters + pagination (viewer+) → 200', async () => {
  const { app } = await withCluster();
  const res = await request(app).get('/api/event-clusters').set('Authorization', authHeader('viewer'));
...
test('GET list requires auth → 401', async () => {
  const { app } = await withCluster();
  assert.equal((await request(app).get('/api/event-clusters')).status, 401);
});

test('GET list rejects an invalid status filter → 400', async () => {
  const { app } = await withCluster();
  const res = await request(app).get('/api/event-clusters?status=bogus').set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 400);
});

test('GET list rejects a bad limit → 400', async () => {
  const { app } = await withCluster();
  const res = await request(app).get('/api/event-clusters?limit=9999').set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 400);
});

test('GET list passes status + time-range + pagination to the repo', async () => {
  const seen = [];
  const eventClustersRepo = makeEventClustersRepo({ list: async (f) => { seen.push(f); return []; }, count: async () => 0 });
  const app = makeApp({ eventClustersRepo });
  const res = await request(app)
    .get('/api/event-clusters?status=open&from=2026-07-01T00:00:00Z&to=2026-07-02T00:00:00Z&limit=10&offset=5')
...
test('GET list returns a clean 500 on a repo failure (no stack leak)', async () => {
  const eventClustersRepo = makeEventClustersRepo({ list: async () => { throw new Error('db exploded'); } });
  const app = makeApp({ eventClustersRepo });
  const res = await request(app).get('/api/event-clusters').set('Authorization', authHeader('viewer'));
...
test('GET /:id is 400 for a non-numeric id', async () => {
  const { app } = await withCluster();
  const res = await request(app).get('/api/event-clusters/abc').set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 400);
});

test('GET /:id is 404 for an unknown cluster', async () => {
  const { app } = await withCluster();
  const res = await request(app).get('/api/event-clusters/99999').set('Authorization', authHeader('viewer'));
...
test('POST /:id/ack acknowledges the cluster (operator+) → 200', async () => {
  const { app, eventClustersRepo, id } = await withCluster();
  const res = await request(app).post(`/api/event-clusters/${id}/ack`).set('Authorization', authHeader('operator'));
  assert.equal(res.status, 200);
  assert.equal(res.body.cluster.status, 'acknowledged');
  assert.equal(eventClustersRepo.rows.find((r) => r.id === id).status, 'acknowledged');
});

test('POST /:id/ack is forbidden for a viewer → 403', async () => {
  const { app, id } = await withCluster();
  const res = await request(app).post(`/api/event-clusters/${id}/ack`).set('Authorization', authHeader('viewer'));
  assert.equal(res.status, 403);
});

test('POST /:id/ack requires auth → 401', async () => {
  const { app, id } = await withCluster();
  assert.equal((await request(app).post(`/api/event-clusters/${id}/ack`)).status, 401);
});

test('POST /:id/ack is 404 for an unknown cluster', async () => {
  const { app } = await withCluster();
  const res = await request(app).post('/api/event-clusters/99999/ack').set('Authorization', authHeader('operator'));
  assert.equal(res.status, 404);
});
...
// ---- POST /api/event-clusters/:id/resolve ------------------------------
...
test('POST /:id/resolve with a note resolves the cluster (operator+) → 200', async () => {
  ...
  const row = eventClustersRepo.rows.find((r) => r.id === id);
...
test('POST /:id/resolve is 404 for an unknown cluster', async () => {
  const { app } = await withCluster();
  const res = await request(app)
    .post('/api/event-clusters/99999/resolve')
    .set('Authorization', authHeader('operator'))
    .send({ note: 'x' });
  assert.equal(res.status, 404);
});
...
test('POST /:id/resolve twice → second is 409', async () => {
  ...
  await request(app).post(`/api/event-clusters/${id}/resolve`).set('Authorization', auth).send({ note: 'first' });
