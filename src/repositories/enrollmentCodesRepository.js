...
// a trusted agent), so it is never kept in cleartext — migration 072 dropped the
...
function createEnrollmentCodesRepository(db, { secretBox = null } = {}) {
  ...
  // nothing to decrypt (a row predating migration 072, which dropped the
  ...
  // `code` is null for a row minted before migration 072 (which dropped the
  ...
  async function findByCode(code) {
    // Matched on the hash — including for codes minted before migration 072,
