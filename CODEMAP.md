...
| 035 | `audit_events` | **unified, server-wide audit** (Reporting → Audit) — user actions (audit middleware) + agent activity (on ingest); recurring activity folded onto one row via a nullable UNIQUE `dedup_key`. Agent activity includes `agent.probe-failed` — a probe the agent could not execute (e.g. `traceroute` missing), with the reason in `detail.reason` — and `agent.error`, a non-fatal operational error the agent reports over the WS channel (recorded in `src/ws/agentSocket.js`, deduped per category) so agent-side failures are visible server-side, not just in the host's local log |
...
| Audit trail (Reporting → Audit) | unified server-wide audit, **admin only**: middleware `src/middleware/auditLogger.js` (records user actions in `res.on('finish')`, mounted in `src/app.js`) + pure helpers `src/audit/actions.js` (`describeRequest`/`redactBody`); agent activity recorded on ingest in `src/routes/agentReports.js` and agent-reported errors (`agent.error`) over the WS channel in `src/ws/agentSocket.js`; repository `src/repositories/auditEventsRepository.js` (`record` discrete / `recordRecurring` repeat-suppressed via UNIQUE `dedup_key`); read router `src/routes/auditEvents.js` (`/api/audit`); table `audit_events` (migration 035); UI `auditModule()` in `public/app.js` (Reporting → Audit). See docs/audit.md |
