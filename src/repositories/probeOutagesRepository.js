...
// Maps a joined outage row to the API shape (camelCase + location/agent names).
function mapRow(row) {
  if (!row) return null;
  return {
    id: Number(row.id),
...
function createProbeOutagesRepository(db) {
  const { pool } = db;

  // The currently-active outage for a (agent, metric, target), or null.
  async function findActive(agentId, metric, target) {
    const [rows] = await pool.query(
      `SELECT id, started_at, severity FROM probe_outages
  ...
  // Opens a new outage. Caller guarantees no active one exists for the tuple.
  async function open({ location_id = null, agent_id, metric, severity, started_at, affected_target }) {
    const [res] = await pool.query(
      `INSERT INTO probe_outages (location_id, agent_id, metric, severity, started_at, affected_target)
       VALUES (?, ?, ?, ?, ?, ?)`,
  ...
  // Raises the severity of an active outage (e.g. warning → critical when a
  // run escalates). Only ever increases — guarded to active rows. Returns true
  // if a row changed.
  async function updateSeverity(id, severity) {
    const [res] = await pool.query(
      `UPDATE probe_outages SET severity = ? WHERE id = ? AND resolved_at IS NULL`,
      [severity, id]
    );
    return res.affectedRows > 0;
  }

  // Resolves an active outage, stamping resolved_at + duration_seconds. The
  ...
  // Lists outages overlapping [from, to], optionally filtered by severity and
  // location. An outage overlaps the window if it started before `to` and was
  ...
  // Outages for ONE agent overlapping [from, to], newest-first. Same overlap
...
module.exports = { createProbeOutagesRepository, mapRow };
